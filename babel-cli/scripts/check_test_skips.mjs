...
const hasLiveProviderKey = hasDeepSeekKey;
...
if (hasLiveProviderKey) {
  console.log('[check-test-skips] CI: DEEPSEEK_API_KEY set — live provider tests may run.');
  process.exit(0);
}

if (allowSkips) {
  console.warn(
    '[check-test-skips] CI: DEEPSEEK_API_KEY missing; BABEL_ALLOW_TEST_SKIPS=1 — skip-only live tests allowed.',
  );
  console.warn(
...
console.error('[check-test-skips] CI=true but DEEPSEEK_API_KEY is unset.');
...
console.error('       DEEPINFRA_API_KEY remains available only for offline/legacy compatibility.');
