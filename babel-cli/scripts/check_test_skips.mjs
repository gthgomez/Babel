...
const hasLiveProviderKey = hasOpenRouterKey;
...
if (hasLiveProviderKey) {
  console.log('[check-test-skips] CI: OPENROUTER_API_KEY set — live provider tests may run.');
  process.exit(0);
}

if (allowSkips) {
  console.warn(
    '[check-test-skips] CI: OPENROUTER_API_KEY missing; BABEL_ALLOW_TEST_SKIPS=1 — skip-only live tests allowed.',
  );
  console.warn(
...
console.error('[check-test-skips] CI=true but OPENROUTER_API_KEY is unset.');
...
console.error('    1. Provide OPENROUTER_API_KEY in CI for preferred test:live-governance / pipeline regressions.');
console.error('       Direct DEEPSEEK_API_KEY remains available only for offline/legacy compatibility.');
