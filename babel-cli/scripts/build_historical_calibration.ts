...
function parseArgs(argv: string[]): { roots: string[]; output: string; help: boolean } {
  const roots: string[] = [];
  let output = '';
  let help = false;
...
function main(): void {
  ...
  if (args.roots.some((root) => !existsSync(root))) {
    throw new Error(`One or more source roots do not exist: ${args.roots.join(', ')}`);
  }

  const build = buildHistoricalCalibrationCorpus({ sourceRoots: args.roots });
  const written = writeHistoricalCalibrationCorpus(build, args.output, args.roots);
  const recovery = {
    schema_version: 1,
    ...
    runs: build.corpus.entries.map((entry) => ({
      ...
      new_classification: build.analyses.find((analysis) => analysis.entry_id === entry.entry_id)?.new_causal_report.attribution ?? null,
    ...
    note: build.corpus.entries.length === 0
  ...
  const gapCounts = new Map<string, { count: number; classifications: Record<string, number> }>();
  for (const analysis of build.analyses) {
    for (const fact of analysis.impossible_or_unknown_facts) {
      const current = gapCounts.get(fact.fact) ?? { count: 0, classifications: {} };
      current.count += 1;
      current.classifications[fact.classification] = (current.classifications[fact.classification] ?? 0) + 1;
  ...
  const reconciliation = {
    ...
    classifications: [
      { class: 'spawn_error', status: 'environment_observed_during_local_verification', live_cells: 0 },
      { class: 'temp_cleanup_error', status: 'environment_observed_during_local_verification', live_cells: 0 },
      { class: 'dataset_or_fixture_issue', status: 'not_observed_in_corpus', live_cells: 0 },
      { class: 'route_failure', status: 'requires_route_receipt_or_unknown', live_cells: 0 },
      { class: 'stale_snapshot', status: 'not_observed_in_corpus', live_cells: 0 },
      { class: 'environment_failure', status: 'reported_only_when_directly_evidenced', live_cells: 0 },
      { class: 'regression', status: 'not_claimed_without_before_after_evidence', live_cells: 0 },
    ],
  ...
  const calibration = {
    ...
    cells: [],
    blockers: ['live provider execution was not requested by this offline command'],
  };
  const comparison = {
    schema_version: 1,
  ...
  const ledger = {
    ...
    status: build.corpus.entries.length > 0 ? 'DERIVED_FROM_HISTORICAL_EVIDENCE' : 'NO_HISTORICAL_EVIDENCE',
    recommendations: [],
  ...
  const reportPaths = [
    ...
    writeReport(args.output, 'runtime-error-reconciliation.json', reconciliation),
  ...
  process.stdout.write(`${JSON.stringify({ manifest: written.manifestPath, analyses: written.analysisPaths.length, reports: reportPaths }, null, 2)}\n`);
}
