...
function findCandidateAppRoot(taskText: string, projectRoot: string): string {
  ...
  const candidateDirs = [projectRoot, ...listDirectories(projectRoot, 2)].filter((candidatePath) =>
    existsSync(join(candidatePath, 'app', 'src', 'main', 'java')),
  );
  if (candidateDirs.length === 0) {
    return projectRoot;
  }
  const ranked: RankedCandidate[] = candidateDirs
    .map((candidatePath) => {
...
function listUiDirectories(appRoot: string): string[] {
  ...
  const uiSubdirs = [javaRoot, ...listDirectories(javaRoot, 6)].filter(
    (candidatePath) => basename(candidatePath).toLowerCase() === 'ui',
  );
...
function listFrontendUiDirectories(projectRoot: string): string[] {
  ...
  return unique(candidates.filter((candidatePath) => existsSync(candidatePath))).sort((a, b) =>
    a.localeCompare(b),
  );
}
...
function findReferenceSourceRoots(taskText: string, projectRoot: string): string[] {
  ...
  return unique(candidates).filter((candidatePath) => {
    ...
    const candidateIdentifier = normalizeIdentifier(candidateName);
    const hintedByName =
      candidateLower.startsWith('reference-') || candidateLower.includes('reference');
    const hintedByTask =
      normalizedTask.includes(candidateLower) ||
      (candidateIdentifier && taskIdentifier.includes(candidateIdentifier));
...
function buildReferenceInventorySnippets(projectRoot: string, referenceRoots: string[]): string[] {
  ...
  for (const referenceRoot of referenceRoots) {
    ...
    const sourceModuleFiles = listGroundedFiles(
      join(referenceRoot, 'monte_carlo_ledger'),
      /\.py$/i,
    );
    if (sourceModuleFiles.length > 0) {
      const sourceModuleNames = unique(
        sourceModuleFiles.map((filePath) => basename(filePath)),
      ).sort((a, b) => a.localeCompare(b));
      snippets.push(
        `// ${pathToPosix(projectRoot, join(referenceRoot, 'monte_carlo_ledger'))} closed source module inventory (exact basenames)\n// ${sourceModuleNames.join(', ')}\n// Do not invent models.py or engine.py; they are not present in this inventory.`,
      );
    }
  }
  return snippets;
...
export function classifyTaskContract(taskText: unknown): TaskContract {
  ...
  const requestsSuggestions =
    /\b(suggest(?:ed)? changes?|recommend(?:ation|ations)?|improvement(?:s)?|next steps)\b/.test(
      normalized,
    );
  const requestsPlan = /\b(plan|roadmap|prioriti[sz]ed)\b/.test(normalized);
  const uiFocus = /\b(ui|ux|screen|screens|compose|material|layout|accessibility|visual)\b/.test(
    normalized,
  );
  const implementationRequested =
    /\b(implement|apply|fix|modify|update|improve|refine|polish|create|port|continue|finish|complete|build)\b/.test(
      normalized,
    ) && /\b(code|ui|screen|screens|app|changes?)\b/.test(normalized);
  const evidenceOnlyAllowed =
    /\b(gather|collect|read|inspect)\b.{0,40}\b(evidence|files|context|inputs)\b/.test(
      normalized,
    ) &&
    !/\b(make|produce|deliver|write|draft)\b.{0,40}\b(plan|recommend|findings|changes|audit|review)\b/.test(
      normalized,
    );
...
export function getAndroidUiAuditHardening(taskContract: TaskContract | null | undefined): {
  ...
  return {
    ...
    requiredSkillIds: unique(['skill_jetpack_compose', 'skill_android_ui_audit_review']),
    requiredTaskOverlayIds: unique(['task_overlay_ai_android_development']),
    projectOverlayId: 'overlay_project_android',
  };
}
...
export function isDeliverableCompletenessReject(
  verdict: Partial<QaVerdict> | null | undefined,
): boolean {
  if (!verdict || !('failures' in verdict) || !Array.isArray(verdict.failures)) return false;
  return verdict.failures.some((failure) =>
    String((failure as { condition?: unknown }).condition ?? '').includes(
...
export function buildTaskGrounding(
  ...
  if (
    (!taskContract?.groundingRequired && !taskContract?.implementationRequested) ||
    !projectRoot ||
  ...
  const frontendUiDirectories =
    taskContract.focusArea === 'ui' ? listFrontendUiDirectories(projectRoot) : [];
  const groundingRoots =
    androidUiDirectories.length > 0
      ? androidUiDirectories
      : frontendUiDirectories.length > 0
        ? frontendUiDirectories
        : [appRoot];
  const filePattern =
    frontendUiDirectories.length > 0 && androidUiDirectories.length === 0
  ...
  if (isRepositoryTask) {
    ...
    for (const entityFile of entityFiles) {
      ...
      entitySnippets.push(
        `// ${relative(projectRoot, entityFile).replace(/\\/g, '/')}\n${content}`,
      );
    }
    const daoFiles = files.filter(
      (filePath) => filePath.endsWith('Dao.kt') || filePath.endsWith('Dao.java'),
    );
    for (const daoFile of daoFiles) {
      ...
      const sigLines = content
        .split('\n')
        .filter((line) =>
          /^\s*(suspend\s+)?fun\s+|@(Query|Insert|Update|Delete|Transaction)/.test(line),
        )
        .join('\n');
...
export function formatGroundingContext(grounding: TaskGrounding | null | undefined): string {
  ...
  const lines = [
    ...
    ...grounding.files.map(
      (filePath) => `  - ${relative(grounding.projectRoot, filePath).replace(/\\/g, '/')}`,
    ),
  ];
  if (grounding.referenceRoots?.length > 0) {
    lines.push(
      ...
      ...grounding.referenceRoots.map(
        (referenceRoot) => `  - ${pathToPosix(grounding.projectRoot, referenceRoot)}`,
      ),
  ...
  if (grounding.companionSnippets?.length > 0) {
    lines.push(
      '',
      'Referenced class signatures (first 60 lines each):',
      grounding.companionSnippets.join('\n---\n'),
    );
  }
  if (grounding.referenceInventorySnippets?.length > 0) {
    lines.push(
      '',
      'Reference source file_read allowlist (closed; use only these exact paths instead of guessing module names):',
      grounding.referenceInventorySnippets.join('\n---\n'),
    );
    lines.push(
      '',
  ...
  if (grounding.entitySnippets?.length > 0) {
    lines.push(
      '',
      'Entity schemas (authoritative field definitions — do not invent other fields):',
      grounding.entitySnippets.join('\n---\n'),
    );
  }
  if (grounding.daoSnippets?.length > 0) {
    lines.push(
      '',
      'DAO method signatures (authoritative callable surface — do not invent other methods):',
      grounding.daoSnippets.join('\n---\n'),
    );
  }
  return lines.join('\n');
}
...
function resolveGroundedTargetMatches(target: unknown, grounding: TaskGrounding): string[] {
  ...
  for (const [basenameKey, relativePaths] of basenameToRelative.entries()) {
    ...
    const pattern = new RegExp(
      `(^|[^A-Za-z0-9_])${escapeRegex(basenameKey)}($|[^A-Za-z0-9_])`,
      'i',
    );
...
function canonicalizeExternalReferenceTarget(
  _target: unknown,
  _grounding: TaskGrounding,
): null {
  return null;
}
...
function canonicalizeReferenceModuleGuess(step: StepLike, grounding: TaskGrounding): string | null {
  ...
  if (normalizedBasename === 'core_engine.py' || normalizedBasename === 'engine.py') {
    ...
    const haystack = [String(step?.description ?? ''), String(step?.rationale ?? '')]
      .join(' ')
      .toLowerCase();
  ...
  for (const candidateBasename of candidateBasenames) {
    const match = grounding.files.find(
      (filePath) => basename(filePath).toLowerCase() === candidateBasename,
    );
    if (match) {
      return match;
    }
...
export function normalizePlanTargetsAgainstGrounding(
  ...
  for (const step of swePlan.minimal_action_set) {
    ...
    if (guessedModuleTarget && String(normalizedStep.target).trim() !== guessedModuleTarget) {
      changed = true;
      warnings.push(
        `[PLAN_TARGET_CANONICALIZED] Step ${step.step} target normalized to grounded reference module: ${guessedModuleTarget}`,
      );
      normalizedStep = { ...normalizedStep, target: guessedModuleTarget };
    }
    const externalCanonicalTarget = canonicalizeExternalReferenceTarget(
      normalizedStep.target,
      grounding,
    );
    if (
      externalCanonicalTarget &&
      String(normalizedStep.target).trim() !== externalCanonicalTarget
    ) {
      changed = true;
      warnings.push(
        `[PLAN_TARGET_CANONICALIZED] Step ${step.step} target normalized to mirrored reference root: ${externalCanonicalTarget}`,
      );
    ...
    if (matches.length === 1) {
      ...
      if (String(normalizedStep.target).trim() !== canonicalTarget) {
        changed = true;
        warnings.push(
          `[PLAN_TARGET_CANONICALIZED] Step ${step.step} target normalized to grounded file: ${canonicalTarget}`,
        );
    ...
    changed = true;
    warnings.push(
      `[PLAN_TARGET_SPLIT] Step ${step.step} target expanded into ${matches.length} grounded single-file steps.`,
    );
...
export function buildGroundedEvidenceRegistry(
  ...
  const availableGroundingFiles = grounding.files.filter((filePath) =>
    availableSet.has(toGroundedRelativePath(grounding, filePath).toLowerCase()),
  );
  ...
  const referencedFiles = Array.isArray(swePlan?.minimal_action_set)
    ? unique(
        swePlan.minimal_action_set.flatMap((step) =>
          resolveGroundedTargetMatches(step?.target, availableGrounding),
        ),
      )
    : [];
  return {
    ...
    availableBasenameSet: new Set(
      availableFiles.map((filePath) => basename(filePath).toLowerCase()),
    ),
  };
}

export function formatGroundedEvidenceRegistry(
  ...
  if (registry.referencedFiles.length > 0) {
    lines.push(
      `Plan-referenced grounded files (${registry.referencedFiles.length}):`,
      ...registry.referencedFiles.map((filePath) => `  - ${filePath}`),
    );
  }
  return lines.join('\n');
}
...
function extractEvidenceGateMentions(condition: unknown): string[] {
  return unique(
    (String(condition ?? '').match(/[A-Za-z0-9_./\\:-]+\.(kt|java|xml|md)/gi) ?? []).map((match) =>
      normalizeEvidencePath(match),
    ),
  );
}
...
function isGroundedEvidenceMention(registry: GroundedEvidenceRegistry, mention: string): boolean {
  ...
  return (
    registry.availableFileSet.has(normalized) ||
    registry.availableBasenameSet.has(basename(normalized).toLowerCase())
  );
}
...
export function reconcileQaVerdictWithGroundedEvidence(
  ...
  for (const failure of verdict.failures) {
    ...
    if (
      mentions.length === 0 ||
      !mentions.every((mention) => isGroundedEvidenceMention(registry, mention))
    ) {
      remainingFailures.push(failure);
      continue;
    }
  ...
  if (remainingFailures.length === 0) {
    return {
      verdict: 'PASS',
      overall_confidence: Math.max(1, Math.min(5, Number(verdict.overall_confidence) || 3)) as
        | 1
        | 2
        | 3
        | 4
...
function extractReferencedPaths(step: StepLike): string[] {
  ...
  if (
    !target.includes(',') &&
    !target.includes('*') &&
    (/\.(kt|java|xml|md)$/i.test(target) || /[\\/]$/.test(target))
  ) {
    return [target];
  }
  ...
  for (const step of swePlan.minimal_action_set) {
    ...
    for (const candidatePath of candidatePaths) {
      ...
      if (
        ...
        (/\.(kt|java|xml)$/i.test(candidatePath) || tool === 'file_read' || tool === 'audit_ui')
      ) {
        violations.push(
          `[GROUNDING_GUARD] Step ${step.step} references missing path: ${candidatePath}`,
        );
        continue;
      }
      if (/\.(kt|java|xml)$/i.test(candidatePath) && !(basenameMatch || inGroundedFiles)) {
        violations.push(
          `[GROUNDING_GUARD] Step ${step.step} references file outside grounded inventory: ${candidatePath}`,
        );
      }
    }
  }
...
export function buildGroundingQaReject(violations: string[]): Partial<QaVerdict> {
  return {
    ...
    proposed_fix_strategy:
      'Regenerate the plan using the grounded file inventory and gather real file content before referencing additional existing files.',
  };
}
