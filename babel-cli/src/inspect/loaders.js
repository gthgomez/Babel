...
const STANDARD_ARTIFACTS = [
    { key: 'manifest', label: 'Manifest', filename: '01_manifest.json', type: 'json', optional: false },
    { key: 'manual_prompt', label: 'Manual SWE Prompt', filename: '02_manual_swe_prompt.md', type: 'markdown', optional: true },
    { key: 'manual_repair', label: 'Manual Plan Repair', filename: '02_manual_plan_repair.md', type: 'markdown', optional: true },
    { key: 'execution_report', label: 'Execution Report', filename: '04_execution_report.json', type: 'json', optional: true },
    { key: 'checkpoints', label: 'Checkpoints', filename: 'checkpoints/checkpoints.json', type: 'json', optional: true },
    { key: 'waterfall', label: 'Waterfall Telemetry', filename: '05_waterfall_telemetry.json', type: 'json', optional: true },
    { key: 'runtime_telemetry', label: 'Runtime Telemetry', filename: '06_runtime_telemetry.json', type: 'json', optional: true },
    { key: 'trace_context', label: 'Trace Context', filename: '07_trace_context.json', type: 'json', optional: true },
    { key: 'routing_decision', label: 'Routing Decision', filename: '08_routing_decision.json', type: 'json', optional: true },
    { key: 'session_context', label: 'Session Context', filename: '10_session_context.json', type: 'json', optional: true },
];
...
function buildSchemaLearningSummary(runDir) {
    ...
    return {
        ...
        latestFailureKind: ledgerEntries.length > 0 && typeof ledgerEntries[ledgerEntries.length - 1].failure_kind === 'string'
            ? ledgerEntries[ledgerEntries.length - 1].failure_kind
            : 'unknown',
        latestFailureStage: ledgerEntries.length > 0 && typeof ledgerEntries[ledgerEntries.length - 1].stage === 'string'
...
function resolveRunDirBySessionId(sessionId, babelRunsDir) {
    ...
    for (const candidate of candidates) {
        ...
        if (manifest && typeof manifest.session_id === 'string' && manifest.session_id === normalizedSessionId) {
            return candidate;
        }
    }
...
export function resolveInspectRunDir(options) {
    ...
    if (!runInput || runInput.toLowerCase() === 'latest') {
        ...
        if (!latest) {
            throw new Error(`No latest run pointer found${options.project ? ` for project ${options.project}` : ''}.`);
        }
        return latest;
    }
    return runInput;
...
function inferLayerTypeFromPath(path) {
    ...
    if (path.includes('\\02_Skills\\') || path.includes('/02_Skills/'))
        return 'skill';
    if (path.includes('\\02_Pipeline_Stages\\') || path.includes('/02_Pipeline_Stages/'))
...
function inferFinalStatus(bundle) {
    return bundle.runtimeTelemetry?.final_outcome ??
...
function inferQaFailureTags(bundle) {
    if (Array.isArray(bundle.runtimeTelemetry?.qa_failure_tags) && bundle.runtimeTelemetry.qa_failure_tags.length > 0) {
        return [...bundle.runtimeTelemetry.qa_failure_tags];
    }
    const latestQa = bundle.qaVerdicts.length > 0
        ? bundle.qaVerdicts[bundle.qaVerdicts.length - 1].data
        : null;
...
function deriveFilesChanged(executionReport) {
    ...
    return [...new Set(executionReport.tool_call_log
            .filter((entry) => entry?.tool === 'file_write' && typeof entry?.target === 'string' && entry.target.length > 0)
...
function deriveTestsRun(executionReport) {
    ...
    return [...new Set(executionReport.tool_call_log
            .filter((entry) => entry?.tool === 'test_run' && typeof entry?.target === 'string' && entry.target.length > 0)
            .map((entry) => entry.target))];
}
function deriveOutcomeStatus(bundle) {
    const executionStatus = bundle.executionReport?.status ?? null;
    const finalOutcome = bundle.runtimeTelemetry?.final_outcome ?? bundle.manifest?.runtime_telemetry?.final_outcome ?? null;
    const warnings = [];
    if (bundle.manifest?.runtime_telemetry?.final_outcome &&
        ...
        warnings.push(`Manifest runtime_telemetry.final_outcome (${bundle.manifest.runtime_telemetry.final_outcome}) differs from 06_runtime_telemetry.json (${bundle.runtimeTelemetry.final_outcome}).`);
    }
    if ((executionStatus === 'EXECUTION_HALTED' || executionStatus === 'ACTIVATION_REFUSED') && finalOutcome === 'COMPLETE') {
        warnings.push(`Execution report status (${executionStatus}) conflicts with runtime final outcome (${finalOutcome}). Outcome is derived conservatively from execution_report.`);
    }
...
export function loadInspectBundle(runDir) {
    ...
    const schemaLearningHintPath = join(dirname(runDir), SCHEMA_LEARNING_DIR, SCHEMA_SHADOW_HINTS_FILENAME);
...
function buildStageDescriptorsForBundle(bundle) {
    ...
    const pipelineMode = bundle.runtimeTelemetry?.pipeline_mode ?? bundle.manifest?.analysis?.pipeline_mode ?? 'unknown';
    const latestQaVerdict = bundle.qaVerdicts.length > 0
    ...
    return [
        {
            ...
            meta: bundle.manifest?.orchestrator_version ? `v${String(bundle.manifest.orchestrator_version).replace('.0', '')}` : undefined,
        },
        {
            index: 2,
            label: 'Planner',
            state: plannerAttempts > 0 || bundle.manualPrompt ? 'PASS' : (bundle.manifest ? 'PENDING' : 'PENDING'),
            meta: plannerAttempts > 0 ? `${plannerAttempts} plan artifact(s)` : (bundle.manualPrompt ? 'manual prompt exported' : undefined),
        },
        {
            index: 3,
            label: 'QA Reviewer',
            state: latestQaVerdict === 'PASS'
                ? 'PASS'
                : finalStatus === 'QA_REJECTED_MAX_LOOPS'
                    ? 'FAIL'
                    : (pipelineMode === 'manual' || pipelineMode === 'direct')
                        ? 'BLOCKED'
                        : qaAttempts > 0
                            ? 'PENDING'
                            : 'PENDING',
            meta: qaAttempts > 0 ? `${qaAttempts} verdict artifact(s)` : (pipelineMode === 'manual' || pipelineMode === 'direct' ? `mode ${pipelineMode}` : undefined),
        },
        {
            index: 4,
            label: 'Executor',
            state: executionStatus === 'EXECUTION_COMPLETE'
                ? 'PASS'
                : executionStatus === 'EXECUTION_HALTED'
                    ...
                    : executionStatus === 'ACTIVATION_REFUSED'
                        ...
                        : pipelineMode === 'autonomous'
                            ? 'PENDING'
                            : 'BLOCKED',
            meta: executionStatus ?? (pipelineMode === 'autonomous' ? undefined : `mode ${pipelineMode}`),
        },
    ];
}
export function buildInspectRunView(bundle) {
    const finalStatus = inferFinalStatus(bundle);
    const task = bundle.manifest?.analysis?.task_summary ?? bundle.manifest?.handoff_payload?.user_request ?? 'Unavailable';
    return {
        kind: 'run',
        runDir: bundle.runDir,
        identity: bundle.identity,
        project: bundle.manifest?.target_project ?? 'unknown',
        mode: bundle.runtimeTelemetry?.pipeline_mode ?? bundle.manifest?.analysis?.pipeline_mode ?? 'unknown',
        ...
        artifactPointers: [
            ...
            { label: 'Stack Source', value: bundle.artifactRecords.find((artifact) => artifact.key === 'manifest')?.path ?? 'Unavailable' },
            { label: 'Manifest', value: bundle.artifactRecords.find((artifact) => artifact.key === 'manifest')?.path ?? 'Unavailable' },
            { label: 'Checkpoints', value: bundle.artifactRecords.find((artifact) => artifact.key === 'checkpoints')?.path ?? 'Unavailable' },
            { label: 'Session Context', value: bundle.artifactRecords.find((artifact) => artifact.key === 'session_context')?.path ?? 'Unavailable' },
            { label: 'Schema Failure Ledger', value: bundle.schemaLearning.ledgerPath ?? 'Unavailable' },
            { label: 'Schema Learning Hints', value: bundle.schemaLearning.shadowHintsPath ?? 'Unavailable' },
            { label: 'Schema Failure Side Artifacts', value: `${bundle.schemaLearning.schemaFailureArtifactsPath} (${String(bundle.schemaLearning.schemaFailureArtifactCount)})` },
        ],
        checkpointCount: Array.isArray(bundle.checkpoints?.checkpoints) ? bundle.checkpoints.checkpoints.length : 0,
        checkpointRestoreHint: 'babel checkpoint list --run "' + bundle.runDir + '"',
        modelContextAvailable: Boolean(bundle.sessionContext),
        modelContextRestoreHint: bundle.sessionContext ? 'babel session resume "' + bundle.runDir + '" --json' : 'Unavailable',
...
export function buildInspectStackView(bundle) {
    ...
    const promptManifest = bundle.manifest?.compiled_artifacts?.prompt_manifest ?? bundle.manifest?.prompt_manifest ?? [];
...
export function buildInspectManifestView(bundle) {
    return {
        ...
        artifactRecords: [
            ...
            ...bundle.files
                .filter((file) => !bundle.artifactRecords.some((artifact) => artifact.filename === file.name))
                .map((file) => ({
                key: file.name,
                label: file.name,
                filename: file.name,
...
export function buildInspectOutcomeView(bundle) {
    const mode = bundle.runtimeTelemetry?.pipeline_mode ?? bundle.manifest?.analysis?.pipeline_mode ?? 'unknown';
    const qaVerdict = inferLatestQaVerdict(bundle);
    const outcomeStatus = deriveOutcomeStatus(bundle);
    const executionStatus = bundle.executionReport?.status ?? null;
    const executionPerformed = Boolean(bundle.executionReport &&
        ((typeof bundle.executionReport.steps_executed === 'number' && bundle.executionReport.steps_executed > 0) ||
            (Array.isArray(bundle.executionReport.tool_call_log) && bundle.executionReport.tool_call_log.length > 0)));
    ...
    return {
        ...
        runOutcome: {
            ...
            purpose_suppression_reason: bundle.manifest?.compiled_artifacts?.purpose_suppression_reason ?? null,
            domain: bundle.runtimeTelemetry?.domain_id ?? bundle.manifest?.instruction_stack?.domain_id ?? null,
