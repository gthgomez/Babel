...
function createPythonReferenceRepo(root: string): void {
  const refDir = join(root, 'reference-montecarlo-ledger');
  mkdirSync(join(refDir, 'monte_carlo_ledger'), { recursive: true });
  mkdirSync(join(refDir, 'docs'), { recursive: true });
  writeFileSync(join(refDir, 'README.md'), '# MonteCarlo Ledger\n');
  writeFileSync(join(refDir, 'pyproject.toml'), '[project]\nname = "monte-carlo-ledger"\n');
  writeFileSync(join(refDir, 'requirements.txt'), 'numpy\npandas\n');
  writeFileSync(join(refDir, 'monte_carlo_ledger', '__init__.py'), '');
  writeFileSync(join(refDir, 'monte_carlo_ledger', 'main.py'), 'def run(): pass\n');
  writeFileSync(join(refDir, 'monte_carlo_ledger', 'models.py'), 'class Model: pass\n');
  writeFileSync(join(refDir, 'monte_carlo_ledger', 'engine.py'), '');
  writeFileSync(join(refDir, 'docs', 'overview.md'), '# Overview\n');
}

test('collectReferenceSourceShapeViolations: returns null when manifest has no project root', () => {
  const result = collectReferenceSourceShapeViolations(makePlan(), {
    target_project: 'global',
  } as never);
  assert.equal(result, null);
});

test('collectReferenceSourceShapeViolations: returns null when reference dir does not look like Python', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-no-python-'));
  try {
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const result = collectReferenceSourceShapeViolations(
      makePlan({
        minimal_action_set: [
          makeStep({ step: 1, tool: 'file_read', target: 'reference-montecarlo-ledger/README.md' }),
        ],
      }),
      manifest,
    );
...
test('collectReferenceSourceShapeViolations: returns null when reference dir does not exist', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-no-dir-'));
  try {
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const result = collectReferenceSourceShapeViolations(
      makePlan({
...
test('collectReferenceSourceShapeViolations: rejects Android mirror file probes inside reference repo', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-android-probe-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const plan = makePlan({
      minimal_action_set: [
        makeStep({
    ...
    const result = collectReferenceSourceShapeViolations(plan, manifest);
    assertRejectShape(result, 1, 'EVIDENCE-GATE', /SOURCE_SHAPE|SOURCE_INVENTORY_MISMATCH/);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('collectReferenceSourceShapeViolations: rejects Android source dir probes', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-android-src-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const plan = makePlan({
      minimal_action_set: [
        makeStep({
          step: 1,
          tool: 'file_read',
          target: './reference-montecarlo-ledger/app/src/main/java/Test.kt',
        }),
      ],
    });
    const result = collectReferenceSourceShapeViolations(plan, manifest);
...
test('collectReferenceSourceShapeViolations: rejects non-inventory module reads in EVIDENCE_REQUEST plans', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-noninv-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    // Without reading README.md and pyproject.toml first, module reads are flagged
    const plan = makePlan({
      ...
      minimal_action_set: [
        makeStep({
          step: 1,
          tool: 'file_read',
          target: 'reference-montecarlo-ledger/monte_carlo_ledger/core_engine.py',
        }),
      ],
    });
    const result = collectReferenceSourceShapeViolations(plan, manifest);
    assert(result !== null, 'expected rejection');
    assert.equal(result.verdict, 'REJECT');
    // Could hit REFERENCE_FILE_READ_DISCIPLINE (read before README+pyproject)
    // or SOURCE_INVENTORY_GUESS (guessing non-inventory basename)
    const condition = result.failures[0]?.condition ?? '';
    const hitExpected =
      /REFERENCE_FILE_READ_DISCIPLINE/.test(condition) || /SOURCE_INVENTORY_GUESS/.test(condition);
    assert.ok(hitExpected, `expected discipline or guess violation, got: ${condition}`);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('collectReferenceSourceShapeViolations: ordering correctly tracks seen README and pyproject', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-order-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const plan = makePlan({
      ...
      minimal_action_set: [
        ...
        makeStep({
          step: 2,
          tool: 'file_read',
          target: 'reference-montecarlo-ledger/pyproject.toml',
        }),
        makeStep({
          step: 3,
          tool: 'file_read',
          target: 'reference-montecarlo-ledger/monte_carlo_ledger/models.py',
        }),
      ],
    });
    const result = collectReferenceSourceShapeViolations(plan, manifest);
    // models.py exists on disk and README+pyproject were read first, so no violation
    assert.equal(result, null);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test('collectReferenceSourceShapeViolations: rejects non-existent file reads inside reference repo', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-nonexist-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const plan = makePlan({
      ...
      minimal_action_set: [
        ...
        makeStep({
          step: 3,
          tool: 'file_read',
          target: 'reference-montecarlo-ledger/monte_carlo_ledger/nonexistent.py',
        }),
      ],
    });
    const result = collectReferenceSourceShapeViolations(plan, manifest);
    assertRejectShape(result, 1, 'EVIDENCE-GATE', /SOURCE_INVENTORY_MISMATCH/);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});

test('collectReferenceSourceShapeViolations: rejects directory_list of non-existent reference directory', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-bad-dir-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const plan = makePlan({
      minimal_action_set: [
        makeStep({
          step: 1,
          tool: 'directory_list',
          target: 'reference-montecarlo-ledger/nonexistent_dir',
        }),
      ],
    });
    const result = collectReferenceSourceShapeViolations(plan, manifest);
...
test('collectReferenceSourceShapeViolations: directory_list of existing reference directory passes', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-ok-dir-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const plan = makePlan({
      minimal_action_set: [
        makeStep({ step: 1, tool: 'directory_list', target: 'reference-montecarlo-ledger/docs' }),
      ],
    });
    const result = collectReferenceSourceShapeViolations(plan, manifest);
    assert.equal(result, null);
...
test('collectReferenceSourceShapeViolations: empty minimal_action_set returns null', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-ref-empty-'));
  try {
    createPythonReferenceRepo(root);
    const manifest = { target_project: 'test', target_project_path: root } as never;
    const result = collectReferenceSourceShapeViolations(makePlan(), manifest);
    assert.equal(result, null);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
