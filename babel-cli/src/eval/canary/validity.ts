 ...
 */
export function verifyCanaryTaskValidity(spec: CanaryTaskSpec, repeats = 2): TaskValidityReceipt {
  const notes: string[] = []
  const verifier_digest = createHash('sha256').update(spec.oracle_test).digest('hex')
  ...
  if (spec.intended_terminal === 'NO_CHANGE_REQUIRED') {
    ...
    notes.push('C09: oracle is expected to fail; block is the intended agent terminal')
  } else if (spec.intended_terminal === 'false_complete_probe') {
    const inadequate = spec.files
      .filter((f) => typeof f.inadequate === 'string')
      .map((f) => ({ relativePath: f.relativePath, contents: f.inadequate as string }))
    ...
    reference_verified = referenceRuns.every((r) => r.hidden_ok)
    notes.push('C10: inadequate patch must pass visible and fail hidden')
  } else {
    baseline_verified = baselineRuns.every((r) => !r.hidden_ok)
    reference_verified = referenceRuns.every((r) => r.hidden_ok)
  }
...
export function isLiveCanaryEligible(receipt: TaskValidityReceipt): boolean {
  return receipt.baseline_verified && receipt.reference_verified && receipt.oracle_stable
}
