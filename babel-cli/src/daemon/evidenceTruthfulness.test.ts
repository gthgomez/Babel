...
import { realQueueWaitMs } from './queue.js';
...
test('queue wait is measured from the job creation timestamp, never fabricated', () => {
  const started = Date.parse('2026-09-14T00:00:10.000Z');
  assert.equal(realQueueWaitMs(started, '2026-09-14T00:00:00.000Z'), 10_000);
  // A wait that would be negative (clock skew) clamps to zero, never negative.
  assert.equal(realQueueWaitMs(started, '2026-09-14T00:00:20.000Z'), 0);
  // Unknown/malformed creation time stays an explicit zero, not NaN.
  assert.equal(realQueueWaitMs(started, null), 0);
  assert.equal(realQueueWaitMs(started, 'not-a-date'), 0);
  assert.ok(Number.isFinite(realQueueWaitMs(started, undefined)));
});
