...
function formatOnboardingHuman(reportPath: string, report: ReturnType<typeof analyzeProjectRoot>): string {
  return [
...
export function registerProjectCommands(program: Command): void {
  ...
  program
    ...
    .option('--commands <commands>', 'Semicolon-separated commands to run instead of detected test/build/lint commands')
...
`)
  ...
  program
    ...
    .argument('[path]', 'Project root to inspect', '.')
    .option('--write-project-context', 'Write the generated PROJECT_CONTEXT.md draft into the target project')
...
`)
    .action((pathArg: string, options: { writeProjectContext?: boolean; force?: boolean; json?: boolean }) => {
      try {
        ...
        if (options.writeProjectContext === true) {
          ...
          if (existsSync(contextPath) && options.force !== true) {
            throw new Error(`PROJECT_CONTEXT.md already exists at ${contextPath}. Use --force to overwrite it.`);
          }
...
`)
    .action((templateArg: string, pathArg: string, options: { force?: boolean; json?: boolean }) => {
      try {
        const template = normalizeProjectTemplate(templateArg);
        if (template === null) {
          throw new Error(`Invalid template "${templateArg}". Valid values: ${listProjectTemplates().join(', ')}`);
        }
