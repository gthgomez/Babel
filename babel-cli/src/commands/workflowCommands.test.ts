...
function writeFixture(runDir: string, options: {
  ...
  writeFileSync(join(runDir, '04_execution_report.json'), JSON.stringify({ status: 'SMALL_FIX_COMPLETE' }), 'utf-8');
  ...
  if (options.withCheckpoint === true) {
    ...
    writeFileSync(join(runDir, 'checkpoints', 'checkpoints.json'), JSON.stringify({
      checkpoints: [{ id: 'cp-quick-fix' }],
    }), 'utf-8');
  }
  if (options.smallFixCheckpointId !== undefined) {
    writeFileSync(join(runDir, 'small_fix_checkpoint.json'), JSON.stringify({
      schema_version: 1,
...
describe('buildSmallFixLitePayload', () => {
  it('renders complete fix output with verification, checkpoints, evidence, and usage ledger path', () => {
    ...
    try {
      ...
      const payload = buildSmallFixLitePayload(makeResult({
        status: 'SMALL_FIX_COMPLETE',
      ...
      assert.equal(payload.checkpoint.restore_command, 'bl undo');
      ...
      const doPayload = buildSmallFixLitePayload(makeResult({
        status: 'SMALL_FIX_COMPLETE',
        changedFiles: ['src/math.js'],
        checks: ['npm test: passed'],
        usageSummary: {
      ...
      const notRequiredPayload = buildSmallFixLitePayload(makeResult({
        status: 'SMALL_FIX_COMPLETE',
        changedFiles: [],
        checks: [],
        usageSummary: {
  ...
  it('renders failed small-fix output with restore unavailable, verification failed, and recovery fields', () => {
    ...
    try {
      writeFixture(runDir);
      writeFileSync(join(runDir, 'small_fix_failure_capsule.json'), JSON.stringify({
      ...
      const human = formatLiteResultHuman(payload);
      assert.match(human, /^Babel Fix Failed/);
      assert.match(human, /Changed:\nnone/);
      assert.match(human, /Verified:\n- npm test: failed/);
      assert.match(human, /Recovery:\nnone required/);
