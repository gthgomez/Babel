...
import {
  buildEventStreamContract,
} from '../services/eventStream.js';
import {
...
Command Guide:
  Short Lite:  bl ask, bl plan, bl propose, bl fix, bl review, bl undo, bl do
  Main CLI:    babel ask, babel plan, babel propose, babel fix, babel review, babel undo, babel do
  Shortcut:    bl "<task...>" or babel "<task...>"
  Advanced:    babel advanced (includes babel run for audit, output, and governed pipeline controls)
  Health:      doctor, dry, permissions
  Sessions:    resume, continue, interactive, inspect, session
  More:        advanced

Permission presets (daily work):
  --read-only   read-only audit; no file edits or shell execution
  default       safe workspace writes with checkpoint on fix
  --ask         prompt before mutating actions
  --auto        trusted workspace auto-edit with policy gates still active

Notes:
  - Use bl for daily work; babel run lives under "babel advanced" for explicit pipeline controls.
  - Babel Lite shorthand is supported: bl ask|plan|propose|fix|review|undo|do "<task...>".
  - Main CLI user verbs are supported: babel ask|plan|propose|fix|review|undo|do "<task...>".
...
const ADVANCED_HELP_GROUPS: Array<[string, string[]]> = [
  ['Daily work', ['ask', 'plan', 'propose', 'diff', 'fix', 'review', 'undo', 'do', 'lite']],
  ['Advanced pipeline', ['run']],
  ['Readiness', ['setup', 'doctor', 'simplify', 'docs', 'dry', 'permissions', 'models']],
  ['Evidence', ['prove', 'learn', 'evidence', 'inspect', 'session', 'checkpoint', 'diagnose', 'stats']],
  ...
  ['Internals', ['internals', 'mcp', 'mode', 'tools', 'events', 'context', 'escalation', 'shadow-diff']],
];
...
function formatHelpGroups(program: Command, title: string, groups: Array<[string, string[]]>): string {
  const lines = [title, ''];
  for (const [group, names] of groups) {
    ...
    for (const name of names) {
      ...
      const description = command.name() === 'run'
        ? 'Advanced pipeline lane (use bl for daily work): explicit modes, audit, output, and tool/model controls'
        : command.description();
...
export function applyUserFocusedHelpTiers(program: Command): void {
  ...
  program
    ...
    .addHelpText('after', `
Notes:
  - Prefer bl ask|plan|propose|fix|review|undo|do for daily work.
  - babel run is for explicit pipeline modes, audit flags, JSON event streams, and governed controls.
  - Permission presets: --read-only (audit), default (safe_repo), --ask (prompt before mutation), --auto (trusted auto-edit).
  - Pipeline mode names (direct, verified, manual, autonomous, parallel_swarm) are documented on "babel run --help".
`)
    .action(() => {
  ...
  const internalsCommand = program
    ...
    .action(() => {
      console.log(formatHelpGroups(program, 'Babel Internal Commands', [
...
function exitWithRuntimeValidationFailure(message: string, jsonOutput: boolean): never {
  if (jsonOutput) {
    ...
    console.error(`[babel] Environment validation failed:\n${message}`);
  }
...
export function printBanner(): void {
  ...
  process.stdout.write(renderProductBanner('Multi-Agent OS Runtime Harness', `${modeTag}${dryTag}`) + '\n');
}
...
function handleDryMode(action: 'status' | 'on' | 'off', options: { json?: boolean }): void {
  try {
    ...
    if (options.json) {
      ...
      console.error(`[babel] Dry mode fatal: ${message}`);
    }
    process.exit(1);
  }
}
...
function describePermissionProfile(status: ApprovalProfileStatus): {
  ...
  if (status.profile === 'full-auto') {
    return {
      action: 'Babel may edit files and run checks inside the trusted workspace without repeated prompts.',
      scope: ['edit in-scope project files', 'run local verifiers such as npm test', 'keep sandbox and provider boundaries active'],
      cost: 'Configured provider calls may use credits; expensive model tiers still require explicit opt-in.',
      approval: 'Only outside-workspace, network, dependency, or high-cost boundaries should interrupt the flow.',
    };
  }
  return {
    action: 'Babel may edit files and run local checks inside the selected project.',
    scope: ['edit in-scope project files', 'run local verifiers such as npm test', 'write recovery evidence for failures'],
    cost: 'Configured provider calls may use credits; expensive model tiers still require explicit opt-in.',
    approval: 'Trusted workspace work should not ask redundant approvals.',
  };
}

export function buildApprovalProfilePayload(status: ApprovalProfileStatus): Record<string, unknown> {
...
function handlePermissionsCommand(profileArg: string | undefined, options: { json?: boolean }): void {
  try {
    ...
    if (!profile) {
      throw new Error(`Invalid approval profile "${profileArg}". Valid values: ${APPROVAL_PROFILES.join(', ')}`);
    }
    ...
    if (options.json) {
      ...
      console.error(`[babel] Permissions fatal: ${message}`);
    }
    process.exit(1);
  }
}
...
function parseApprovalStatus(value: string | undefined): ApprovalStatus | 'all' {
  ...
  throw new Error(`Invalid approval status "${value}". Valid values: all, ${APPROVAL_STATUSES.join(', ')}`);
}
...
function printApprovalRequestRequired(
  ...
  printJsonOrHuman(payload, `${formatApprovalHuman(record)}\n\nNext: babel approvals approve ${record.id}`, json);
}

function parseValidMode(value: string | undefined, fallback: ValidMode = 'verified'): ValidMode {
  const normalized = String(value ?? fallback).trim().toLowerCase();
  if (VALID_MODES.includes(normalized as ValidMode)) {
    return normalized as ValidMode;
  }
  throw new Error(`Invalid mode "${value}". Valid values: ${VALID_MODES.join(', ')}`);
}
...
async function runAgentJobNow(jobId: string): Promise<AgentJob> {
  ...
  try {
    ...
    const result = await withJobEnv(running, () => runBabelPipeline(running.task, {
      mode: running.mode,
      ...(running.model ? { modelOverride: running.model as never } : {}),
      ...(running.model_tier ? { modelTier: running.model_tier } : {}),
      ...(running.execution_profile ? { executionProfile: running.execution_profile as never } : {}),
      ...(running.model_tier === 'escalation' ? { allowExpensive: true } : {}),
      eventBus,
    }));

    const verification = result.status === 'COMPLETE' && running.project_root
    ...
    const status = completionGate.status === 'fail'
      ? 'verification_failed'
...
async function handleModelsPing(options: { model?: string; json?: boolean; allowExpensive?: boolean }): Promise<void> {
  ...
  try {
    ...
    const payload = {
      ...
      stream_idle_timeout_ms: Number(process.env['BABEL_DEEPINFRA_STREAM_IDLE_TIMEOUT_MS'] ?? '60000'),
    ...
    if (options.json) {
      ...
      console.log(`Model ping passed: ${payload.backend_key} (${payload.provider_model_id}) in ${payload.latency_ms}ms`);
    }
  } catch (err: unknown) {
    const payload = {
      status: 'fail',
    ...
    if (options.json) {
      ...
      console.error(`[babel] Model ping failed: ${payload.error}`);
    }
    process.exit(1);
  }
}
...
function printExecutorToolList(options: {
  ...
  const showCatalog = options.policy === true ||
  ...
  if (options.json) {
    process.stdout.write(`${JSON.stringify({
      ...
      ...(showCatalog
        ? {
            execution_profile: options.executionProfile ?? process.env['BABEL_EXECUTION_PROFILE'] ?? 'safe_repo',
            catalog,
          }
        : {}),
    }, null, 2)}\n`);
  ...
  for (const tool of tools) {
    ...
    console.log(`  ${tool.name.padEnd(16)} ${tool.category.padEnd(12)} ${safety.padEnd(9)} ${tool.description}`);
  }
}
...
function printExecutorToolInspect(tool: ExecutorToolSnapshot, json: boolean): void {
  ...
  console.log(`  Required: ${tool.input.required.length > 0 ? tool.input.required.join(', ') : '(none)'}`);
  console.log(`  Optional: ${tool.input.optional.length > 0 ? tool.input.optional.join(', ') : '(none)'}`);
  console.log(`  Policy:   ${tool.policyTags.join(', ')}`);
  console.log(`  ${tool.description}`);
}

function handleExecutorToolInspect(name: string, options: { json?: boolean; policy?: boolean; executionProfile?: string }): void {
  ...
  if (!tool) {
    if (options.json) {
      ...
      console.error(`[babel] Unknown executor tool "${name}". Run "babel tools list" to inspect available tools.`);
    }
    process.exit(1);
  }
  ...
  if (options.policy === true) {
    ...
    if (options.json === true) {
      process.stdout.write(`${JSON.stringify({ status: 'ok', tool, catalog_entry: catalogEntry }, null, 2)}\n`);
      return;
    }
...
function printSessionList(sessions: SessionSummary[], json: boolean): void {
  if (json) {
    process.stdout.write(`${JSON.stringify({ status: 'ok', count: sessions.length, sessions }, null, 2)}\n`);
    return;
  }
...
function handleInspectMode(
  ...
  try {
    ...
    console.error(`[babel] Inspect fatal: ${err instanceof Error ? err.message : String(err)}`);
    process.exit(1);
  }
}

function resolveProofRunArg(runArg: string | undefined, options: { last?: boolean; run?: string }): string {
...
function resolveProofRunDir(runArg: string | undefined, options: { last?: boolean; run?: string; project?: string }): string {
...
function handleLearnFromRun(
  runArg: string | undefined,
  options: { run?: string; project?: string; last?: boolean; json?: boolean; learningRoot?: string },
): void {
  try {
    const resolvedRunDir = resolveProofRunDir(runArg, options);
    const proofArtifacts = writeProofArtifacts(resolvedRunDir);
    const learningRoot = options.learningRoot ? resolve(options.learningRoot) : join(BABEL_ROOT, 'learning');
    ...
    printJsonOrHuman(payload, formatLearningFailureHuman(learningArtifacts.record), options.json === true);
  } catch (err: unknown) {
    printJsonErrorAndExit(err instanceof Error ? err.message : String(err), options.json === true);
  }
}
...
function handleLearnInspect(
  ...
  try {
    ...
    const payload = artifact.kind === 'failure'
      ? {
          status: 'ok',
          learning_root: learningRoot,
    ...
    const human = artifact.kind === 'failure'
...
function handleLearnPackage(
  ...
  try {
    ...
    printJsonOrHuman(payload, formatMutationPackageHuman(result.mutationPackage), options.json === true);
  } catch (err: unknown) {
    printJsonErrorAndExit(err instanceof Error ? err.message : String(err), options.json === true);
  }
}
...
function resolveRunForReadOnlyCommand(run: string | undefined, project: string | undefined): string {
...
function printSetupChecklist(options: { json?: boolean }): void {
  ...
  for (const item of payload.first_five_minutes) {
    console.log(`  ${item.step}: ${item.command}`);
  }
}

function printSessionResume(runArg: string | undefined, options: { project?: string; json?: boolean }): void {
  try {
...
export function registerCoreCommands(program: Command): void {
  program
    .command('resolve')
    .description('Resolve a Babel Local Mode instruction stack using the canonical TypeScript resolver')
    .option('--task-category <category>', 'Task category: frontend | backend | compliance | devops | research | mobile | game', 'frontend')
    ...
    .option('--pipeline-mode <mode>', 'Pipeline mode: direct | verified | autonomous', 'direct')
    .option('--codex-adapter <adapter>', 'Codex adapter preference: auto | balanced | ultra', 'auto')
    ...
    .option('--purpose-mode <mode>', 'Purpose mode: execution | verification | learning | exploration | audit')
    ...
    .action((options: {
      ...
      try {
        const result = resolveLocalStack({
          ...
          pipelineMode: (options.pipelineMode ?? 'direct') as LocalPipelineMode,
    ...
    .action(async (options: { project?: string }) => {
      ...
      if (options.project) {
        ...
        if (resolved) {
          ...
          console.warn(`[babel] Warning: Could not resolve root for project "${options.project}". Falling back to current directory.`);
        }
      }
      ...
      if (!shadowRoot) {
        console.error('[babel] BABEL_SHADOW_ROOT is not set. Enable dry-mode shadowing first.');
        process.exit(1);
      }

      const result = getShadowDiff(shadowRoot, projectRoot);
      if (result.status === 'error') {
        console.error(`[babel] Shadow diff failed: ${result.error}`);
        process.exit(1);
      }
...
`)
  ...
  const approvalsCommand = program
    .command('approvals')
    .description('Manage approval requests for installs, unattended jobs, and expensive model escalation')
    .addHelpText('after', `
Examples:
...
`)
  ...
  approvalsCommand
    ...
    .action((id: string, options: { json?: boolean }) => {
      ...
      printJsonOrHuman({ status: 'ok', approval: record }, formatApprovalHuman(record), options.json === true);
    });
  ...
  approvalsCommand
    ...
    .option('--project-root <path>', 'Project root scope')
    .option('--json', 'Emit structured JSON only')
    .action((options: { task?: string; model?: string; modelTier?: string; projectRoot?: string; json?: boolean }) => {
      try {
...
`)
  ...
  jobsCommand
    .command('create')
    .description('Create a resumable example_autonomous_agent/Babel job')
  ...
  jobsCommand
    ...
    .action((options: { json?: boolean }) => {
      const payload = listAgentJobs();
      printJsonOrHuman({ status: 'ok', ...payload }, formatAgentJobListHuman(payload), options.json === true);
    });
  ...
  jobsCommand
    ...
    .action(async (id: string, options: { json?: boolean }) => {
      try {
        ...
        printJsonOrHuman({ status: job.status, job }, formatAgentJobHuman(job), options.json === true);
  ...
  jobsCommand
    ...
    .action((id: string, options: { json?: boolean }) => {
      try {
        ...
        printJsonOrHuman({ status: 'ok', job: reported }, formatAgentJobHuman(reported), options.json === true);
      } catch (err: unknown) {
        printJsonErrorAndExit(err instanceof Error ? err.message : String(err), options.json === true);
      }
    });
  ...
  diagnoseCommand
    ...
    .action((runDir: string, options: { json?: boolean }) => {
      ...
      printJsonOrHuman({ status: 'ok', diagnosis }, formatHaltDiagnosisHuman(diagnosis), options.json === true);
    });
  ...
  diagnoseCommand
    ...
    .action((id: string, options: { json?: boolean }) => {
      ...
      const diagnosis = job.diagnosis ?? diagnoseRun({
        runDir: job.run_dir,
...
`)
    .action((runArg: string | undefined, options: { last?: boolean; run?: string; project?: string; json?: boolean }) => {
...
`)
  ...
  learnCommand
    ...
    .option('--json', 'Emit structured JSON only')
    .action((runArg: string | undefined, options: {
      last?: boolean;
      run?: string;
      project?: string;
      learningRoot?: string;
  ...
  learnCommand
    ...
    .option('--learning-root <path>', 'Directory for learning artifacts')
    .option('--json', 'Emit structured JSON only')
    .action((lessonId: string, options: { shadow?: boolean; learningRoot?: string; json?: boolean }) => {
  ...
  learnCommand
    ...
    .argument('<lesson-id>', 'Lesson candidate id or direct candidate path')
    .requiredOption('--target <target>', 'Mutation target type: project-verifier-contract or project-overlay')
    .option('--learning-root <path>', 'Directory for learning artifacts')
    .option('--json', 'Emit structured JSON only')
    .action((lessonId: string, options: { target?: string; learningRoot?: string; json?: boolean }) => {
      handleLearnPackage(lessonId, options);
    });

  learnCommand
    .command('inspect')
    .description('Read a learning failure, lesson candidate, shadow lesson, eval record, or mutation package')
    .argument('<artifact-id>', 'Failure id, lesson id, mutation id, or direct learning artifact path')
    .option('--learning-root <path>', 'Directory for learning artifacts')
    .option('--json', 'Emit structured JSON only')
...
`)
  ...
  toolsCommand
    ...
    .option('--capabilities', 'Include capability broker entries such as archive and bundle inspection')
    .option('--execution-profile <profile>', 'Evaluate policy/capabilities for an execution profile')
    .option('--allowed-tools <tools>', 'Comma-separated run-level allowed tool names to simulate')
    .option('--disallowed-tools <tools>', 'Comma-separated run-level disallowed tool names to simulate')
    .action((options: {
      json?: boolean;
      policy?: boolean;
      whyDisabled?: boolean;
  ...
  toolsCommand
    ...
    .action((name: string, options: { json?: boolean; policy?: boolean; executionProfile?: string }) => {
    ...
    .action((options: { project?: string; json?: boolean }) => {
      ...
      if (!latest) {
        if (options.json) {
          ...
          console.error(`[babel] No latest session pointer found${options.project ? ` for ${options.project}` : ''}.`);
        }
        process.exit(1);
      }
...
`)
    .action(async (options: { last?: boolean; report?: boolean; run?: string; project?: string; json?: boolean }) => {
  ...
  addInspectCommonOptions(inspectCommand.command('report')
    .argument('[run]', 'Run directory or latest')
    .description('Generate proof_status.json and BABEL_RUN_REPORT.md for a Babel run')
    .option('--last', 'Use latest run pointer')
    .option('--json', 'Emit structured JSON only')
    .action(async (runArg: string | undefined, options: { run?: string; project?: string; last?: boolean; json?: boolean }) => {
      handleProofReport(runArg, options);
    }));

  addInspectCommonOptions(inspectCommand.command('run')
    .argument('[run]', 'Run directory or latest')
  ...
  addInspectCommonOptions(inspectCommand.command('summary')
    .argument('[run]', 'Run directory or latest')
  ...
  addInspectCommonOptions(inspectCommand.command('stack')
    .argument('[run]', 'Run directory or latest')
  ...
  addInspectCommonOptions(inspectCommand.command('manifest')
    .argument('[run]', 'Run directory or latest')
  ...
  addInspectCommonOptions(inspectCommand.command('outcome')
    .argument('[run]', 'Run directory or latest')
...
`);
  ...
  checkpointCommand
    ...
    .action((options: { run?: string; project?: string; json?: boolean }) => {
      try {
        ...
        console.error(`[babel] Checkpoint fatal: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
      }
    });
  ...
  program
    ...
    .option('--mode <mode>', 'Default mode for this session', 'verified')
    .addHelpText('after', `
...
`)
    .action(async (options: { project?: string; mode?: string }) => {
      try {
        ...
        console.error(`[babel] Interactive fatal: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
      }
    });
...
`)
    .action(async () => {
      try {
        ...
        console.error(`[babel] MCP fatal: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
      }
    });
  ...
  mcpCommand
    ...
    .option('--json', 'Emit structured JSON only')
    .action((name: string, commandValue: string, args: string[] | undefined, options: { json?: boolean }) => {
      try {
        ...
        if (options.json) {
          ...
          console.error(`[babel] MCP fatal: ${message}`);
        }
        process.exit(1);
      }
    });
  ...
  eventsCommand
    ...
    .option('--json', 'Emit structured JSON only')
    .action((runArg: string | undefined, options: { project?: string; contractOnly?: boolean; json?: boolean }) => {
      try {
  ...
  scheduleCommand
    ...
    .option('--json', 'Emit structured JSON only')
    .action((id: string, jobType: string, options: {
      projectRoot?: string;
      baseRef?: string;
      branch?: string;
      message?: string;
      ...
      try {
        ...
        printJsonOrHuman({ status: 'ok', schedule }, `Created schedule ${schedule.id}`, options.json === true);
      } catch (err: unknown) {
        printJsonErrorAndExit(err instanceof Error ? err.message : String(err), options.json === true);
      }
    });
  ...
  scheduleCommand
    .command('run-now')
    .description('Run a schedule immediately; mutating jobs require --allow-mutate and execute in an isolated project copy')
    .argument('<id>', 'Schedule id')
...
`)
  ...
  scheduleCommand
    ...
    .action((id: string, options: { json?: boolean }) => {
      ...
      printJsonOrHuman(result, result.deleted ? `Deleted schedule ${id}` : `No schedule found for ${id}`, options.json === true);
    });
...
`)
  ...
  registerGitDraftCommand('diff-summary', 'diff_summary', 'Draft a diff summary without mutating Git');
  registerGitDraftCommand('commit-draft', 'commit_draft', 'Draft a commit message without committing');
  ...
  gitCommand
    ...
    .option('--json', 'Emit structured JSON only')
    .action((options: { projectRoot?: string; message?: string; stage?: string; json?: boolean }) => {
      try {
        const stage = options.stage === 'tracked' || options.stage === 'all' ? options.stage : 'staged';
  ...
  gitCommand
    ...
    .option('--json', 'Emit structured JSON only')
    .action((options: { projectRoot?: string; title?: string; body?: string; allowRemote?: boolean; json?: boolean }) => {
      try {
  ...
  program
    ...
    .option('--verify <command>', 'Alias for --check; repeat for multiple required checks', collectOption, [])
    .option('--dry-run', 'Preview only without local or remote mutations (default unless --apply is passed)')
    ...
    .option('--no-pr', 'Do not open a draft PR; keeps remote mutation disabled in the current workflow')
    .option('--json', 'Emit structured JSON only')
    .addHelpText('after', `
Examples:
...
Hard stops include secrets, protected branches, generated artifacts, mixed/unrelated scopes, and failed verification.
`)
    .action((options: {
      projectRoot?: string;
      branch?: string;
  ...
  benchmarkCommand
    ...
    .action((runArg: string | undefined, options: { json?: boolean; benchmarksRoot?: string; suite?: string }) => {
      try {
        ...
        printJsonOrHuman(analysis, formatBenchmarkRunAnalysisHuman(analysis), options.json === true);
      } catch (err: unknown) {
  ...
  benchmarkCommand
    .command('repair')
    .description('Generate a focused benchmark repair plan and prompt from a failed Terminal-Bench run')
    .argument('[run]', 'Run directory, result.json path, or latest', 'latest')
    .option('--json', 'Emit structured JSON only')
    .option('--benchmarks-root <path>', 'Benchmarks workspace root')
    .option('--suite <name>', 'Terminal-Bench suite for latest lookup', 'pilot10')
  ...
  benchmarkCommand
    ...
    .option('--execution-profile <profile>', 'Execution profile for repair mode', 'benchmark_container')
    .option('--deepinfra-timeout-ms <n>', 'DeepInfra per-request timeout for repair and targeted rerun', '240000')
    .option('--waterfall-timeout-ms <n>', 'Aggregate waterfall timeout for repair and targeted rerun', '720000')
    ...
    .option('--dry-run', 'Prepare workspace and commands without running Babel, Docker verifier, or targeted benchmark')
    .option('--skip-babel-repair', 'Do not run Babel repair mode; useful for verifying an existing workspace/checkpoint')
    ...
    .option('--fail-on-unresolved', 'Exit non-zero unless the loop reaches a local or targeted pass')
    .action(async (runArg: string | undefined, options: {
      json?: boolean;
      benchmarksRoot?: string;
      suite?: string;
      maxTasks?: string;
  ...
  benchmarkCommand
    ...
    .option('--deepinfra-timeout-ms <n>', 'DeepInfra per-request timeout for generated benchmark commands', '240000')
    .option('--waterfall-timeout-ms <n>', 'Aggregate waterfall timeout for generated benchmark commands', '720000')
    .option('--deadline <iso>', 'Wall-clock deadline for generated benchmark commands')
    .option('--min-remaining-ms <n>', 'Minimum remaining deadline budget before starting benchmark work', '0')
    .option('--job-slug <slug>', 'Stable job slug used in generated benchmark commands', 'improvement-loop')
    .option('--output-dir <path>', 'Loop report artifact output directory')
    .option('--skip-local-checks', 'Inspect benchmark history without running local readiness commands')
    ...
    .action((options: {
      ...
      try {
        ...
        printJsonOrHuman(report, formatBenchmarkImprovementLoopHuman(report), options.json === true);
        if (report.local_readiness.status === 'fail' || (options.failOnUnready === true && report.readiness_gate.status === 'fail')) {
          process.exit(1);
        }
      } catch (err: unknown) {
        printJsonErrorAndExit(err instanceof Error ? err.message : String(err), options.json === true);
...
export function resolveBenchmarkAnalyzeRun(raw: string, options: { benchmarksRoot?: string; suite: string }): string {
  if (raw !== 'latest') {
    return resolve(raw);
  }
  ...
  const latest = collectBenchmarkResultPaths(resultRoot)
    .map((path) => ({ path, mtimeMs: statSync(path).mtimeMs, metadata: readBenchmarkResultMetadata(path) }))
    .filter((entry) => entry.metadata.isJobResult)
    .filter((entry) => entry.metadata.suite === null || entry.metadata.suite === options.suite)
    .sort((left, right) => right.mtimeMs - left.mtimeMs)[0];
  if (!latest) {
    throw new Error(`No Terminal-Bench result.json files found for suite ${options.suite} under ${resultRoot}`);
  }
  return latest.path;
}
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
      ...
      return {
        ...
        isJobResult: record['summary'] !== undefined && (Array.isArray(record['results']) || Array.isArray(record['trials'])),
      };
    }
  } catch {
