...
import {
  formatDocsAuditHuman,
  runDocsAudit,
} from '../services/docsFitness.js';
import {
...
export function registerMaintenanceCommands(program: Command): void {
  program
    ...
    .option('--write-report [path]', 'Write maintenance_audit.json and maintenance_audit.md under runs/maintenance or at the given JSON path')
    .addHelpText('after', `
Examples:
  $ babel simplify
  $ babel simplify --all
...
`)
    .action((target: string | undefined, options: { all?: boolean; json?: boolean; writeReport?: string | boolean }) => {
      try {
        ...
        const report = options.writeReport !== undefined
