...
test('token usage audit generates the expected summary shape without writing artifacts', () => {
  ...
  assert.equal(
    result.latestJsonPath.endsWith('artifacts\\token-audit\\latest\\token-usage-audit.json') ||
      result.latestJsonPath.endsWith('artifacts/token-audit/latest/token-usage-audit.json'),
    true,
  );
});
