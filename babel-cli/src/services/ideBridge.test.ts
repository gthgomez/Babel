...
test('IDE bridge contract is read-only and view-complete', () => {
  ...
  assert.equal(contract.approval_actions, 'session_only');
  assert.equal(contract.host_mode, 'enhanced_repl');
  assert.match(contract.approval_semantics, /interactive REPL/);
...
test('IDE bridge snapshot indexes run evidence and checkpoints without mutation actions', () => {
  ...
  try {
    ...
    assert.equal(
      snapshot.run_timeline.find((item) => item.file === '01_manifest.json')?.present,
      true,
    );
    assert.equal(
      snapshot.run_timeline.find((item) => item.file === '03_qa_verdict.json')?.present,
      false,
    );
    assert.equal(snapshot.plan_review.plan_path, join(root, '02_swe_plan.json'));
    assert.equal(snapshot.diffs.restore_available, true);
    assert.deepEqual(
      snapshot.checkpoint_list.map((item) => item.name),
      ['before.patch'],
    );
