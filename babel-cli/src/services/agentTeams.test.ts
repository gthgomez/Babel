...
function tempRoot(name: string): string {
  const root = join(tmpdir(), `babel-agent-teams-${name}-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
...
test('subagent isolation contract keeps live workers gated behind evidence requirements', () => {
  ...
  assert.equal(contract.live_subagents_enabled, false);
  ...
  assert.ok(contract.required_before_live_subagents.includes('per_agent_evidence_path'));
});
...
test('live subagent state is explicitly opt-in with isolation/evidence/restore requirements', () => {
  ...
  assert.equal(run.live_subagents.enabled, false);
  assert.equal(run.live_subagents.required_opt_in, 'future_explicit_live_subagents');
  ...
  assert.deepEqual(run.live_subagents.required_before_live_subagents, buildSubagentIsolationContract().required_before_live_subagents);
  assert.equal(run.lead_synthesis.live_subagents.required_before_live_subagents.length, buildSubagentIsolationContract().required_before_live_subagents.length);
});
...
test('two subagents can write disjoint scoped files and merge with evidence', () => {
  ...
  assert.equal(run.live_subagents.enabled, false);
  assert.equal(run.lead_synthesis.live_subagents.enabled, false);
  assert.equal(run.agents.length, 2);
  assert.deepEqual(run.diagnostics.filter((diagnostic) => diagnostic.severity === 'fail'), []);
    ...
    agents: [
      {
        ...
        operations: [
          ...
          { type: 'write_file', path: 'created.txt', content: 'new\n', rationale: 'Create new file.' },
        ],
      },
    ],
  }, { babelRoot: root, runsRoot });
...
test('agent merge restore refuses when merged files changed after merge', () => {
  ...
  assert.equal(restore.diagnostics.some((diagnostic) => diagnostic.code === 'merge_restore_target_modified'), true);
...
test('overlapping write scopes are rejected before merge', () => {
  ...
  assert.equal(run.status, 'failed');
  assert.equal(run.diagnostics.some((diagnostic) => diagnostic.code === 'write_scope_conflict'), true);
  const report = mergeAgentRun('team-conflict', { babelRoot: root, runsRoot });
  assert.equal(report.status, 'failed');
  assert.equal(report.diagnostics.some((diagnostic) => diagnostic.code === 'run_failed'), true);
});
