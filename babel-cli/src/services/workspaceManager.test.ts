...
import {
  getexample_autonomous_agentApprovedRoots,
  listWorkspaceFiles,
  readWorkspaceFile,
...
function withApprovedRoots<T>(roots: string[], run: () => T): T {
  const previous = process.env['BABEL_example_autonomous_agent_APPROVED_ROOTS'];
  process.env['BABEL_example_autonomous_agent_APPROVED_ROOTS'] = roots.join(';');
  try {
    return run();
  } finally {
    if (previous === undefined) {
      delete process.env['BABEL_example_autonomous_agent_APPROVED_ROOTS'];
    } else {
      process.env['BABEL_example_autonomous_agent_APPROVED_ROOTS'] = previous;
    }
  }
}
...
test('example_autonomous_agent workspace roots can be overridden for tests', () => {
  const root = mkdtempSync(join(tmpdir(), 'babel-opencalw-roots-'));
  try {
    withApprovedRoots([root], () => {
      const roots = getexample_autonomous_agentApprovedRoots();
...
test('example_autonomous_agent workspace roots default to the whole workspace for user-shaped repo access', () => {
  ...
  try {
    ...
    withWorkspaceRoot(workspace, () => {
      ...
      assert.throws(
        ...
        /outside example_autonomous_agent approved workspace roots/,
      );
    });
  } finally {
...
test('example_autonomous_agent workspace roots expose sandbox mirror aliases', () => {
...
test('workspace file read accepts example_autonomous_agent sandbox mirror aliases for approved roots', () => {
