...
test("Babel Full writes read-only Spark evidence and hardened plan artifacts", () => {
  ...
  assert.ok(result.foundation_artifacts);
  assert.equal(result.foundation_artifacts?.mutation_subagents_enabled, false);
  for (const path of Object.values(result.foundation_artifacts?.paths ?? {})) {
  ...
  assert.equal(
    result.spark_agents.every((agent) =>
      /[\\\/]spark[\\\/]/.test(agent.evidence_path),
    ),
    true,
  );
  assert.match(
    readFileSync(result.hardened_plan_path, "utf-8"),
    /governed Babel executor/i,
  );

  const cartographerEvidence = JSON.parse(
    readFileSync(
      join(result.run_dir, "spark", "read-only", "repo-cartographer.json"),
      "utf-8",
    ),
  ) as {
    repo_cartography?: { package_scripts?: string[]; dir_samples?: string[] };
  };
  assert.ok(cartographerEvidence.repo_cartography);
  assert.deepEqual(cartographerEvidence.repo_cartography?.package_scripts, []);
  assert.ok(
    (cartographerEvidence.repo_cartography?.dir_samples?.length ?? 0) >= 0,
  );
});
...
test("Spark parallel review writes synthesis without mutating reviewers", () => {
  ...
  assert.equal(
    existsSync(join(result.run_dir, "spark_parallel_review.json")),
    true,
  );
  const synthesis = JSON.parse(
    readFileSync(result.synthesis_path, "utf-8"),
  ) as {
...
test("Babel Full can disable read-only agents while preserving route artifacts", () => {
  ...
  const routeDecision = routeLiteOrFull(
    "Run the full lane for this architecture task",
    {
      requestedVerb: "full",
    },
  );
