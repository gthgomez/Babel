...
interface ShipRuntime {
  runCiReview: (options: { projectRoot?: string; baseRef?: string; outputDir?: string; now?: Date }) => CiReviewReport;
  runGitDraft: (kind: 'commit_draft' | 'pr_draft', options: { projectRoot?: string; baseRef?: string; outputDir?: string; now?: Date }) => GitDraftReport;
...
function normalizedScope(path: string): string {
  ...
  return normalized.startsWith('./') ? normalized.slice(2).split('/')[0] ?? '.' : normalized.split('/')[0] ?? '.';
}
...
function defaultRuntime(): ShipRuntime {
  return {
    runCiReview: ({ projectRoot, baseRef, outputDir, now }) => runCiReview({
      projectRoot: projectRoot ?? process.cwd(),
      outputDir: outputDir ?? join(BABEL_RUNS_DIR, 'ci-review'),
      ...(baseRef ? { baseRef } : {}),
      ...(now ? { now } : {}),
    }),
    runGitDraft: (kind, { projectRoot, baseRef, outputDir, now }) => runGitDraft(kind, {
      projectRoot: projectRoot ?? process.cwd(),
      outputDir: outputDir ?? join(BABEL_RUNS_DIR, 'git-drafts'),
      ...(baseRef ? { baseRef } : {}),
      ...(now ? { now } : {}),
    }),
    createGitCommit: ({ projectRoot, outputDir, now, message }) => createGitCommit({
      projectRoot: projectRoot ?? process.cwd(),
    ...
    createGitPullRequest: ({ projectRoot, outputDir, now, title, body }) => createGitPullRequest({
      projectRoot: projectRoot ?? process.cwd(),
      outputDir: outputDir ?? join(BABEL_RUNS_DIR, 'git-mutations'),
      ...(now ? { now } : {}),
      ...(title ? { title } : {}),
...
function makeStop(code: string, message: string, path?: string): ShipHardStop {
  return path ? { severity: 'hard_stop', code, message, path } : { severity: 'hard_stop', code, message };
}
...
function collectHardStops(input: {
  ...
  if (!input.allowMain && isProtectedBranch(input.targetBranch)) {
    stops.push(makeStop('main_branch', 'AGENTS hard-stop: direct shipping to main/master is blocked unless --allow-main is passed.'));
  }

  const scopes = collectTopLevelScopes(input.ci.changed_files);
  if (!input.allowMixed && scopes.length > 1) {
    stops.push(makeStop('mixed_concerns', `Changed paths span multiple scopes: ${scopes.join(', ')}. Use --allow-mixed to proceed.`));
  }

  if (input.ci.changed_files.some((file) => isGeneratedChange(file.path))) {
    stops.push(makeStop('generated_artifact_changed', 'AGENTS hard-stop: generated/build artifacts changed.'));
  }
  ...
  const sourceOnly = input.ci.test_signals.some((signal) => signal.code === 'source_without_test_change');
  if (sourceOnly && input.checkCommands.length === 0) {
    stops.push(makeStop('verification_required', 'Source-only change requires --check command(s) before apply.'));
  }
...
function pushBranch(deps: ShipRuntime, repoRoot: string, branch: string): ShipAction {
  ...
  return {
    ...
    status: result.status === 0 ? 'applied' : 'failed',
    command: ['git', ...args],
    message: result.status === 0 ? `Pushed ${branch} to origin.` : (result.stderr || result.stdout || 'git push failed').trim(),
  };
}

function refreshGitState(report: ShipReport, repoRoot: string, runGit: ShipRuntime['runGit']): void {
...
function buildNext(report: ShipReport): string[] {
  ...
  if (report.outputs.commit_hash) {
    return [report.outputs.pushed_branch
...
function resolveTargetBranch(rawBranch: string | undefined, branchBefore: string | null): string | null {
...
export function runShip(raw: ShipOptions, overrides: Partial<ShipRuntime> = {}): ShipReport {
  ...
  if (repoRootResult.status !== 0) {
    throw new Error(`Unable to resolve repository root: ${(repoRootResult.stderr || repoRootResult.stdout).trim()}`);
  }
  ...
  const checkCommands = [...new Set((raw.checkCommands ?? []).map((command) => command.trim()).filter(Boolean))];
  ...
  const commit = deps.createGitCommit({
    ...
    message: raw.message ?? commitDraft.commit_draft?.subject ?? 'chore(repo): ship reviewed changes',
  });
  report.actions.push({
    name: 'commit',
    status: commit.action.status === 'applied' ? 'applied' : (commit.action.status === 'planned' ? 'planned' : 'failed'),
  ...
  report.outputs.branch = targetBranch || deps.runGit(['branch', '--show-current'], repoRoot).stdout.trim() || null;
  ...
  report.actions.push({
    name: 'pr',
    status: pr.action.status === 'applied' ? 'applied' : (pr.action.status === 'planned' ? 'planned' : 'failed'),
