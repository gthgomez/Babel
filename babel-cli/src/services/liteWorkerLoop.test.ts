...
const skipIfCiWithoutKeys =
  process.env['CI'] === 'true' &&
  !process.env['DEEPSEEK_API_KEY'] &&
  !process.env['DEEPINFRA_API_KEY'];

function describeIf(condition: boolean, ...args: Parameters<typeof test.describe>): ReturnType<typeof test.describe> | void {
  if (condition) return;
  return test.describe(...args);
}
...
function writeTrustDemoRepo(root: string, implementation: string, targetFile: string): void {
  ...
  assert.equal(
    spawnSync('git', ['add', '.'], { cwd: root, encoding: 'utf-8', env: gitEnv }).status,
    0,
  );
  assert.equal(
    spawnSync('git', ['commit', '-m', 'init'], { cwd: root, encoding: 'utf-8', env: gitEnv })
      .status,
    0,
  );
}
...
test(
  ...
  async () => {
    ...
    assert.deepEqual(
      ...
      ['babel_plan', 'babel_propose', 'babel_fix'],
    );
    assert.equal(result.worker_steps.length, 3);
  },
);

test(
  'AgentSession worker chain completes on trust demo fixture',
...
test(
  'runLiteWorkerLoop reports failure when review step cannot run',
  { concurrency: false },
  async () => {
    const fixture = readLiteTrustDemoFixture(resolveLiteTrustDemoFixturePath());
