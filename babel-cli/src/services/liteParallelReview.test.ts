...
test('shouldSpawnSparkReview is true for complex bl do routes only', () => {
  ...
  assert.equal(shouldSpawnSparkReview(decision, { requestedVerb: 'fix', agentsMode: 'read-only' }), false);
});
...
test('inferDoExecutionVerb maps complex tasks to plan or fix', () => {
  ...
  assert.equal(
    inferDoExecutionVerb('Fix the XSS vulnerability in pipeline.ts security sandbox across the repo'),
    'fix',
  );
});

test('lite parallel review harness passes on fixture repos', { concurrency: false, timeout: 120_000 }, async () => {
