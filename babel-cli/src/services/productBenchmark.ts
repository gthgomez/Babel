  ...
  return [
    {
      ...
      userScenario: 'A new user installs or builds Babel and needs to discover the first safe command.',
      benchmarkQuestion: 'Does top-level help start quickly and point to daily commands without requiring prior Babel knowledge?',
      marketBar: 'Codex, Gemini CLI, and Claude Code all provide a quick first command discovery path.',
      marketSources: [MARKET_SOURCES.codexCodeGeneration, MARKET_SOURCES.geminiCli, MARKET_SOURCES.claudeOverview],
    ...
    },
    {
      ...
      userScenario: 'A user wants to know whether this workspace is ready before trusting Babel with code.',
      benchmarkQuestion: 'Can Babel emit machine-readable readiness diagnostics from a fresh command path?',
      ...
      capabilityStatus: 'implemented',
      notes: ['Exit code 1 is accepted here so an unhealthy workspace still records a baseline artifact.'],
    },
    {
      ...
      category: 'onboarding',
      matrixDimension: 'first_run_time',
      userScenario: 'A new user wants one safe command that explains how to install, build, run doctor, and perform a no-mutation probe.',
      benchmarkQuestion: 'Can Babel show a structured first-run checklist without mutating files or starting a model call?',
      marketBar: 'Claude Code, Codex, and Gemini CLI all optimize for fast first-run orientation and clear next commands.',
      marketSources: [MARKET_SOURCES.claudeOverview, MARKET_SOURCES.codexCodeGeneration, MARKET_SOURCES.geminiCli],
      targetOutcome: 'Babel setup emits Windows-first commands for dependency install, build, doctor, and a safe context preview probe.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Setup is read-only and intended as the first local trust-building command before a real task run.'],
    },
    {
      ...
      userScenario: 'A user wants to inspect or change how much autonomy Babel has before running a task.',
    ...
    },
    {
      ...
      marketBar: 'Safe experimentation requires users to know whether mutations are live or shadowed.',
      marketSources: [MARKET_SOURCES.codexCodeGeneration, MARKET_SOURCES.geminiCli, MARKET_SOURCES.claudeOverview],
    ...
    },
    {
      ...
      userScenario: 'A user connects external context through MCP and needs to diagnose setup without running an agent.',
      benchmarkQuestion: 'Can Babel report MCP transport, auth, timeout, and schema-readiness state?',
      marketBar: 'Market CLIs expose MCP configuration, setup diagnostics, resource/prompt discovery, and bounded tool loading.',
      marketSources: [MARKET_SOURCES.geminiCli, MARKET_SOURCES.claudeOverview, MARKET_SOURCES.codexCodeGeneration],
      targetOutcome: 'Babel can report configured MCP servers, transport/auth hints, timeout policy, and lazy schema loading without starting a run.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 4 labels MCP resources, prompts, and tool discovery as untrusted external content; HTTP/OAuth remain explicitly later.'],
    },
    {
      ...
      userScenario: 'A user has run Babel and needs to find evidence, inspection commands, and latest run state.',
      ...
      marketSources: [MARKET_SOURCES.codexCodeGeneration, MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
    ...
    },
    {
      ...
      marketBar: 'Gemini-style checkpoint/restore and Claude-style checkpoint/worktree flows set recovery expectations.',
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Automatic pre-mutation checkpoints with list, inspect, restore, and session resume commands.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Babel exposes checkpoint list/inspect/restore; file_write uses target snapshots and shell_exec/test_run use bounded filesystem-diff restore with inspectable coverage and force-safety warnings.'],
    },
    {
      ...
      category: 'recovery',
      matrixDimension: 'restore_reliability',
      userScenario: 'A user resumes interrupted work and needs continuation commands from the latest run.',
      ...
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'A run id can be resolved back to evidence, checkpoint, and continuation commands.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Exit code 1 is accepted when no latest run exists in a fresh checkout; runs that reach Stage 4 expose a model_context summary.'],
    },
    {
      ...
      category: 'recovery',
      matrixDimension: 'restore_reliability',
      userScenario: 'A user needs to recover not only files but also the agent context needed to continue.',
      benchmarkQuestion: 'Does Babel expose session/model context through resume surfaces?',
      marketBar: 'Gemini checkpoint/restore and Claude checkpoint/worktree flows preserve more than file state.',
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Restore semantics cover file state, executor history, model context, and approval state.',
      command: appendCliArgs(cli, ['session', 'resume', 'latest', '--json']),
      expectation: {
        exitCodes: [0, 1],
        jsonStdout: true,
      ...
      capabilityStatus: 'implemented',
      notes: ['Babel persists 10_session_context.json with executor prompt, history, file-read cache, tool log, and QA approval state; exit code 1 is accepted when no latest run exists.'],
    },
    {
      ...
      userScenario: 'A team wants to extend Babel with local tools or hooks without weakening trust policy.',
      benchmarkQuestion: 'Can Babel diagnose runtime plugin manifests and trust gates?',
      marketBar: 'Codex, Gemini CLI, and Claude Code all expose plugin, extension, skill, or hook surfaces.',
      marketSources: [MARKET_SOURCES.geminiExtensions, MARKET_SOURCES.claudeHooks, MARKET_SOURCES.codexCodeGeneration],
      targetOutcome: 'Plugins can contribute tools, slash commands, MCP bundles, skills, and hooks under explicit trust policy.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 10 keeps plugin trust levels explicit and diagnoses disabled, missing, blocked, and unsafe plugin states before runtime activation.'],
    },
    {
      ...
      benchmarkQuestion: 'Can Babel show hook event, action, and trust level without activating the plugin?',
      marketBar: 'Extension ecosystems need inspectable hook behavior and trust boundaries before activation.',
      marketSources: [MARKET_SOURCES.geminiExtensions, MARKET_SOURCES.claudeHooks, MARKET_SOURCES.codexCodeGeneration],
      targetOutcome: 'The sample format hook exposes local-mutating trust, PostToolUse wiring, and trim_trailing_whitespace behavior as JSON.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Hook execution writes 09_plugin_events.jsonl during runs; this scenario keeps the benchmark read-only by inspecting the manifest contract.'],
    },
    {
      ...
      userScenario: 'An admin wants strict mode to prove Babel is running under explicit managed policy before release.',
      benchmarkQuestion: 'Can Babel verify strict enterprise controls from a managed policy source?',
      marketBar: 'Enterprise coding agents expose managed settings, policy controls, telemetry posture, and safety defaults.',
      marketSources: [MARKET_SOURCES.claudeOverview, MARKET_SOURCES.codexCodeGeneration, MARKET_SOURCES.geminiCli],
      targetOutcome: 'Strict enterprise doctor passes only with a managed policy and reports tool, MCP, network, model, plugin, telemetry, and redaction controls.',
      command: appendCliArgs(cli, ['doctor', '--scope', 'enterprise', '--strict-enterprise', '--json']),
      expectation: {
        exitCodes: [0],
        jsonStdout: true,
        stdoutIncludes: ['enterprise_policy.present_for_strict', 'enterprise_policy.telemetry_opt_in', 'enterprise_policy.redaction'],
      },
      capabilityStatus: 'implemented',
      notes: ['Phase 11 keeps strict enterprise policy opt-in and proves telemetry/redaction controls without changing the personal default path.'],
    },
    {
      ...
      userScenario: 'A user wants multiple agents to work on distinct slices without stomping each other.',
      benchmarkQuestion: 'Can Babel expose agent-team specs and prior runs with write-scope evidence?',
      ...
      targetOutcome: 'Subagents declare role, task, allowed/disallowed tools, read/write scope, evidence path, and synthesis rules while live subagents remain gated.',
      command: appendCliArgs(cli, ['agents', 'contract', '--json']),
      expectation: {
        exitCodes: [0],
        jsonStdout: true,
        stdoutIncludes: ['live_subagents_enabled', 'review_only_agents_cannot_write', 'per_agent_evidence_path'],
      },
      capabilityStatus: 'implemented',
      notes: ['Phase 8 is review-only by default: the deterministic contract exposes evidence requirements and blocks live LLM subagents until future opt-in.'],
    },
    {
      ...
      category: 'agent_teams',
      matrixDimension: 'parallel_agent_review',
      userScenario: 'A lead needs to merge isolated subagent edits only when restore evidence exists.',
      benchmarkQuestion: 'Does Babel expose a restore command for files changed by an agent-team merge?',
      marketBar: 'Parallel editing needs explicit merge, conflict refusal, and rollback evidence before live autonomy expands.',
      marketSources: [MARKET_SOURCES.claudeSubagents, MARKET_SOURCES.codexCodeGeneration],
      targetOutcome: 'Agent-team merge evidence includes a pre-merge snapshot and a visible restore command.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 9 uses isolated copies/worktrees plus merge restore snapshots; live LLM-backed mutating subagents remain gated.'],
    },
    {
      ...
      userScenario: 'A daily user needs discoverable interactive commands for recovery and inspection.',
      benchmarkQuestion: 'Does interactive help expose the recovery/session surfaces?',
      marketBar: 'Market CLIs expose discoverable slash commands for recovery, tools, MCP, plugins, agents, and session state.',
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Babel REPL help exposes checkpoint, restore, session, and agent team command surfaces.',
      command: appendCliArgs(cli, ['interactive', '--help']),
      expectation: {
        exitCodes: [0],
        stdoutIncludes: ['Interactive slash command map:', '/checkpoint, /restore, /session', '/mcp, /plugins, /plugin, /agents'],
      },
      capabilityStatus: 'implemented',
      notes: ['Phase 2 tightens command discoverability across command-specific help and the REPL workflow guide.'],
    },
    {
      ...
      category: 'session_ux',
      matrixDimension: 'context_loading',
      userScenario: 'A user needs to attach a file or directory to a task without loading the whole repo.',
      benchmarkQuestion: 'Can Babel preview bounded context attachments before a run?',
      marketBar: 'Market CLIs support lightweight file and directory context attachment from the prompt surface.',
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview, MARKET_SOURCES.codexCodeGeneration],
      ...
      capabilityStatus: 'implemented',
      notes: ['Context injection resolves inside the project root, skips git-ignored/binary/dependency artifacts, and writes 00_context_injections.json for real runs.'],
    },
    {
      ...
      userScenario: 'A UI or IDE bridge needs machine-readable lifecycle events instead of scraping text.',
      benchmarkQuestion: 'Does Babel expose a stable event stream contract for non-terminal consumers?',
      marketBar: 'IDE/webview bridges need machine-readable run lifecycle events before a UI is built.',
      marketSources: [MARKET_SOURCES.codexCodeGeneration, MARKET_SOURCES.claudeOverview, MARKET_SOURCES.geminiCli],
      targetOutcome: 'Babel exposes a namespaced JSONL event stream hook and a read-only bridge contract for stage, agent, log, result, and error events.',
      ...
      capabilityStatus: 'implemented',
      notes: ['The event stream uses schema_version 2 envelopes with source, sequence, event_type, and payload fields; Phase 9 adds a read-only schema command for IDE/webview prototypes.'],
    },
    {
      ...
      category: 'session_ux',
      matrixDimension: 'successful_task_completion',
      userScenario: 'A user wants to understand latency, tool use, cache behavior, and session state after a run.',
      benchmarkQuestion: 'Can Babel derive stats from evidence without starting a new model call?',
      marketBar: 'Market CLIs expose stats for latency, tools, cache, token use, and session state.',
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Babel derives waterfall latency, tool counts, cache hits, tokens, cost, and session context from evidence bundles.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Exit code 1 is accepted when no latest run exists; populated runs expose derived stats from existing evidence artifacts.'],
    },
    {
      ...
      marketBar: 'Market tools support non-interactive automation, CI, GitHub workflows, or scheduled tasks.',
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview, MARKET_SOURCES.geminiCli],
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 6C implements a local schedule registry and run-now evidence for read-only/draft jobs only; no daemon or mutating scheduled jobs are enabled.'],
    },
    {
      ...
      category: 'automation',
      matrixDimension: 'automation_reliability',
      userScenario: 'A user wants scheduled mutation only when isolation and explicit opt-in are present.',
      benchmarkQuestion: 'Does the schedule run-now help expose the mutating schedule gate?',
      marketBar: 'Higher-autonomy scheduled mutation requires isolation, review gates, and explicit merge semantics.',
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Scheduled mutating work only runs inside isolated roots and merges reviewed paths explicitly.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Mutating schedule jobs require run-now --allow-mutate and execute in an isolated project copy; daemon/recurrence remains future work.'],
    },
    {
      ...
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Babel writes deterministic changed-file, risk, test-signal, and PR-draft evidence without mutating the repo.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 7 adds machine-readable read-only/draft-first delivery policy evidence; CI review writes evidence only under runs/ci-review.'],
    },
    {
      ...
      category: 'automation',
      matrixDimension: 'automation_reliability',
      userScenario: 'A user wants PR text and commit messaging without committing or touching remotes.',
      benchmarkQuestion: 'Can Babel draft Git/PR delivery metadata as evidence?',
      marketBar: 'Market tools support Git and PR workflows, but safe local defaults should draft before mutating.',
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Babel drafts diff summaries, commit messages, and PR metadata as evidence without committing, pushing, or opening PRs.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 6B is draft-only: no branch creation, git commit, push, or PR creation is performed.'],
    },
    {
      ...
      benchmarkQuestion: 'Does Babel expose Git mutation commands with visible remote-gate language?',
      marketBar: 'Market tools can create branches, commits, and PRs, but serious local agents need explicit side-effect gates.',
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Babel exposes local branch/commit mutation with evidence and keeps PR remote creation gated by default.',
      ...
      capabilityStatus: 'implemented',
      notes: ['Branch and commit creation are explicit local mutations with artifacts; PR creation remains planned unless --allow-remote is supplied.'],
    },
    {
      ...
      category: 'automation',
      matrixDimension: 'automation_reliability',
      userScenario: 'A user wants one command to turn verified local work into a branch, commit, push, and draft PR only when hard-stop gates pass.',
      benchmarkQuestion: 'Does Babel expose a guarded ship command with dry-run and hard-stop language?',
      marketBar: 'Codex and Claude Code expose GitHub workflows; Babel needs one AGENTS.md-aligned delivery command with evidence and hard stops.',
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'Babel documents a single ship workflow that writes evidence and refuses unsafe GitHub delivery conditions.',
      ...
      capabilityStatus: 'implemented',
      notes: ['The ship workflow composes CI review, Git drafts, verification, local commit, push, and draft PR creation behind AGENTS hard-stop checks.'],
    },
    {
      ...
      userScenario: 'A maintainer wants to preview the public export without touching the public repo.',
      benchmarkQuestion: 'Can Babel prove public export planning is non-mutating before release?',
      marketBar: 'Public release workflows need repeatable validation, scrub checks, and safe preview commands.',
      marketSources: [MARKET_SOURCES.codexCi, MARKET_SOURCES.claudeOverview],
      targetOutcome: 'The export dry-run validates manifest/source paths, prints planned operations, and performs no writes, deletes, copies, checks, or Git mutations.',
      command: [
        'pwsh',
        '-NoProfile',
        '-ExecutionPolicy',
      ...
      capabilityStatus: 'implemented',
      notes: ['Phase 10 adds dry-run coverage to public release validation and product benchmarking while leaving Babel-public untouched.'],
    },
    {
      ...
      userScenario: 'A daily user wants read-only planning before a narrow fix on an ordinary repo task.',
      benchmarkQuestion: 'Does Babel Lite emit PLAN_READY from bl plan on a repeatable parity fixture repo?',
      marketBar: 'Cursor and Claude Code expose explicit plan/read-only modes before mutating edits.',
      marketSources: [MARKET_SOURCES.claudeOverview, MARKET_SOURCES.codexCodeGeneration],
      targetOutcome: 'Fixture-based internal scorecard records PLAN_READY on the parity corpus repo.',
      ...
      notes: ['Fixture-based internal scoring via scripts/score_lite_feature.ts; not external parity evidence.'],
    },
    {
      ...
      category: 'daily_worker',
      matrixDimension: 'parallel_agent_review',
      userScenario: 'A user wants read-only Spark reviewers on plan/fix tasks without live subagent mutation.',
      ...
      marketSources: [MARKET_SOURCES.claudeSubagents, MARKET_SOURCES.codexCodeGeneration],
      targetOutcome: 'Fixture harness records read-only Spark review metadata and synthesis evidence.',
    ...
    },
    {
      ...
      benchmarkQuestion: 'Can Babel list checkpoints as JSON immediately after a fixture fix completes?',
      marketBar: 'Cursor-style checkpoint/restore UX expects visible recovery surfaces after mutation.',
      marketSources: [MARKET_SOURCES.geminiCommands, MARKET_SOURCES.claudeOverview],
      ...
      capabilityStatus: 'partial',
      notes: ['Scores checkpoint list surface only; full restore parity remains separately evidenced.'],
    },
    {
      ...
      benchmarkQuestion: 'Does bl fix require verifier pass before FIX_COMPLETE on the parity fixture?',
      marketBar: 'Serious coding agents gate completion on verifier/test evidence, not model confidence alone.',
      ...
      capabilityStatus: 'partial',
      notes: ['Offline_demo fixture scope; arbitrary-repo verifier discipline still YELLOW elsewhere.'],
    },
  ];
}
...
function runScenario(
  ...
  for (const expectedText of scenario.expectation?.stdoutIncludes ?? []) {
    ...
    checks.push({
      ...
      message: found ? `stdout included "${expectedText}"` : `stdout did not include "${expectedText}"`,
    });
  }
  ...
  const status: BenchmarkScenarioStatus = checks.some((check) => check.status === 'fail') ? 'fail' : 'pass';
...
function runReadinessCommand(
  ...
  const output = limitOutput(`${result.stdout ?? ''}${result.stderr ?? ''}`, options.maxOutputBytes).text.trim();
  const exitCode = result.status ?? (result.signal ? 1 : 0);
  return {
    id: options.id,
...
function buildReadinessGateScenario(options: {
  ...
  const typecheck = runReadinessCommand(
    [npmExecutable, 'run', 'typecheck'],
    { ...options, id: 'release_readiness.source_typecheck' },
  );
  ...
  return {
    ...
    user_scenario: 'A maintainer needs the product benchmark to fail when source or workspace readiness is unhealthy.',
    benchmark_question: 'Does the product benchmark refuse a green result when source typecheck or doctor readiness is red?',
    market_bar: 'A credible release benchmark must include build/source health and workspace diagnostics, not only command-surface smoke tests.',
    ...
    timed_out: false,
    stdout: JSON.stringify({ checks: checks.map((check) => ({ id: check.id, status: check.status })) }, null, 2),
    stderr: null,
    stdout_truncated: false,
    stderr_truncated: false,
    checks,
...
export function runProductBenchmark(options: ProductBenchmarkOptions = {}): ProductBenchmarkReport {
  ...
  const scorecardSchemaPath = join(BABEL_ROOT, 'docs', 'market-research', 'product-scorecard.schema.json');
  ...
  const scenarios = scenarioDefinitions.map((scenario) => runScenario(scenario, {
    timeoutMs,
    maxOutputBytes,
    cliRoot,
  }));
...
export function formatProductBenchmarkHuman(report: ProductBenchmarkReport): string {
  ...
  lines.push(`Executable pass/fail: ${report.summary.pass} pass, ${report.summary.fail} fail, ${report.summary.not_implemented} not implemented`);
  lines.push(`Capability baseline: ${report.summary.implemented} implemented, ${report.summary.partial} partial, ${report.summary.gap} gap, ${report.summary.not_started} not started`);
  lines.push('');
  for (const scenario of report.scenarios) {
    const duration = scenario.duration_ms === null ? '' : ` (${scenario.duration_ms.toFixed(1)} ms)`;
