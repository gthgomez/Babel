...
const HARD_TASK_PATTERNS: Array<{ code: string; pattern: RegExp; reason: string }> = [
  {
    ...
    pattern:
      /\b(performance|optimi[sz]e|latency|speed|timeout|fast enough|slow|benchmark|throughput)\b/i,
    reason: 'Task mentions performance, speed, timeout, or benchmark-sensitive work.',
  },
  {
    code: 'algorithmic_reverse_engineering',
    pattern:
      /\b(reverse engineer|decomp|compressor|decompress|codegolf|eigenval|scheduler|batching|arc-agi|strategy|corewar)\b/i,
    reason:
      'Task resembles algorithmic reverse engineering, search, compression, or strategy work.',
  },
  {
    code: 'exploit_or_bypass_construction',
    pattern: /\b(exploit|bypass|filter|payload|xss|sandbox escape|injection)\b/i,
    reason:
      'Task asks for exploit-like, bypass, payload, or filter behavior in an isolated workflow.',
  },
  {
    code: 'repeated_halt_or_verifier_failure',
    pattern:
      /\b(executor_halted|qa_rejected|max_loops|verifier failed|complete but failed|segfault|wrong answer)\b/i,
...
export function recommendModelEscalation(input: {
  ...
  const signals = HARD_TASK_PATTERNS.filter((candidate) => candidate.pattern.test(haystack)).map(
    (candidate) => ({
      code: candidate.code,
...
export function formatEscalationRecommendationHuman(
  recommendation: ModelEscalationRecommendation,
): string {
