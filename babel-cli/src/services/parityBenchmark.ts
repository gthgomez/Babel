...
export type ParityComparisonVerdict = 'babel_stronger' | 'competitor_stronger' | 'tie' | 'inconclusive';
...
function defaultTasks(): ParityTaskDefinition[] {
  return [
    ...
    },
    {
      ...
      success_criteria: ['failing test passes', 'no unrelated broad refactor', 'failure evidence is preserved'],
    ...
    },
    {
      ...
      success_criteria: ['uses issue context', 'produces reviewable diff', 'does not invent requirements'],
    ...
    },
    {
      ...
      success_criteria: ['visual issue verified', 'fix rendered in browser', 'no layout regression'],
    ...
    },
    {
      ...
      success_criteria: ['bad change restored', 'unrelated edits preserved', 'restore command documented'],
...
function manualResult(taskId: string, tool: ParityToolId): ParityToolResult {
  return {
    ...
    evidence_path: null,
    notes: ['Measured result not supplied. Add this cell through --fixture before making parity claims.'],
  };
}

function readFixture(path: string): ParityToolResult[] {
  if (!existsSync(path)) {
    throw new Error(`Parity benchmark fixture not found: ${path}`);
  }
  const parsed = FixtureSchema.parse(JSON.parse(readFileSync(path, 'utf8')) as unknown);
...
function mergeResults(tasks: ParityTaskDefinition[], tools: ParityToolId[], supplied: ParityToolResult[]): ParityToolResult[] {
  const byKey = new Map<string, ParityToolResult>();
  for (const result of supplied) {
    byKey.set(resultKey(result), result);
  }
  return tasks.flatMap((task) => tools.map((tool) => byKey.get(`${task.id}::${tool}`) ?? manualResult(task.id, tool)));
}

function compareStatus(babel: ParityToolResult, competitor: ParityToolResult): ParityComparisonVerdict {
  const comparable = [babel.status, competitor.status].every((status) => status === 'success' || status === 'failure');
  if (!comparable) {
    return 'inconclusive';
  }
...
function buildComparisons(tasks: ParityTaskDefinition[], results: ParityToolResult[]): ParityComparison[] {
...
function summarize(tasks: ParityTaskDefinition[], tools: ParityToolId[], results: ParityToolResult[], comparisons: ParityComparison[]): ParityBenchmarkReport['summary'] {
  ...
  const hasBabelStrongerAxis = comparisons.some((comparison) => comparison.verdict === 'babel_stronger');
  return {
    tasks: tasks.length,
    tools: tools.length,
...
function buildTruthfulGapList(comparisons: ParityComparison[]): string[] {
  const gaps = comparisons
    .filter((comparison) => comparison.verdict === 'competitor_stronger' || comparison.verdict === 'inconclusive')
    .map((comparison) => `${comparison.task_id} vs ${comparison.competitor}: ${comparison.verdict}`);
  return gaps.length > 0 ? gaps : ['No competitor-stronger or inconclusive axes were recorded in supplied results.'];
}
...
export function formatParityBenchmarkHuman(report: ParityBenchmarkReport): string {
  return [
    ...
    '',
    'Comparisons:',
    ...report.comparisons.map((comparison) => `${comparison.verdict.toUpperCase().padEnd(20)} ${comparison.task_id} vs ${comparison.competitor}`),
  ].join('\n');
}
