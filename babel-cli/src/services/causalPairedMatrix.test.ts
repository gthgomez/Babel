...
describe("runProviderFreePairedMatrix", () => {
  test("matrix produces all Stage 1 arms per block", () => {
    ...
    assert.equal(block.arm_order.length, CAUSAL_STAGE1_ARMS.length);
    assert.deepEqual(
    ...
    // Fixed order default: control → shadow → enforce → raw baseline
  ...
  test("model parity ok includes block in estimate", () => {
    ...
    for (const arm of CAUSAL_STAGE1_ARMS) {
      assert.equal(
        block.arms[arm]!.observed_model_id,
        DEFAULT_OBSERVED_MODEL_ID,
      );
    }
  ...
  test("resource parity fields present and spend_parity_ok by default", () => {
    ...
    for (const arm of CAUSAL_STAGE1_ARMS) {
      ...
      assert.equal(
        block.arms[arm]!.simulated_cost_usd,
        DEFAULT_SIMULATED_COST_USD,
      );
    }
    ...
    assert.equal(
      defaults.blocks[0]!.resource_parity.equal_model_response_allowance
        .max_turns,
      DEFAULT_MAX_TURNS,
    );
    assert.equal(
  ...
  test("honesty catch block wires honesty_catch true", () => {
    ...
    assert.equal(report.blocks.length, 2);
    const honesty = report.blocks.find(
      (b) => b.task_id === "fixture_paired_honesty",
    );
    assert.ok(honesty);
  ...
  test("shuffleArmOrder produces deterministic permutation for seed", () => {
    ...
    // Still contains all Stage 1 arms
    assert.deepEqual(
    ...
    // At minimum both remain valid Stage 1 permutations
    assert.equal(c.blocks[0]!.arm_order.length, CAUSAL_STAGE1_ARMS.length);
  });
  ...
  test("writes paired-matrix-report.json when evidenceDir set", () => {
    ...
    assert.equal(
      loaded.paired_counts.total_blocks,
      report.paired_counts.total_blocks,
    );
  });
});
