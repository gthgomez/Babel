...
test(
  "runDogfoodApply shadow mode writes promote artifact without mutating live root",
  { concurrency: false },
  async () => {
    ...
    try {
      ...
      const artifact = JSON.parse(
        readFileSync(result.promoteArtifactPath, "utf-8"),
      ) as {
...
test(
  "promoteDogfoodRun copies changed files into live project root",
  { concurrency: false },
  async () => {
