...
export function runGovernanceBenchmark(
  options: GovernanceBenchmarkRunOptions,
): GovernanceBenchmarkRunSummary {
  ...
  if (!task) {
    throw new Error(
      `Unknown benchmark case "${options.caseId}". Use --list to see valid task IDs.`,
    );
  }
  ...
  if (!adapter) {
    throw new Error(
      `Unknown tool "${options.tool}". Configured tools: ${adapters.map((entry) => entry.id).join(', ')}`,
    );
  }

  const runCount = Number.isFinite(options.runs) && options.runs > 0 ? Math.floor(options.runs) : 1;
  const artifactRoot = resolve(
    options.artifactDir ?? join(dirname(resolve(options.outputPath)), 'babel-benchmark-artifacts'),
  );
  ...
  for (let index = 1; index <= runCount; index += 1) {
    const record =
      adapter.id === 'babel'
        ? runBabelBenchmarkTask({
            ...
            runIndex: index,
            artifactRoot,
            ...(options.fixtureRootBase !== undefined
              ? { fixtureRootBase: options.fixtureRootBase }
              : {}),
          })
    ...
    if (schemaErrors.length > 0) {
      throw new Error(
        `Benchmark result failed JSON schema validation:\n${schemaErrors.join('\n')}`,
      );
    }
    records.push(parsed);
  }
...
function buildUnavailableToolRecord(input: {
  ...
  const result: GovernanceBenchmarkResult = {
    ...
    metrics: {
      ...
      prompt_injection_resisted: null,
      exact_instruction_fidelity:
        input.task.category === 'exact_instruction_drift' ? null : 'not_applicable',
    },
    evidence: {
...
function setupBenchmarkFixture(task: BenchmarkTask, fixtureRootBase?: string): string {
  ...
  writeText(
    join(fixtureRoot, 'test', 'math.test.js'),
    [
  ...
  writeText(
    join(fixtureRoot, 'verify-exact.mjs'),
    [
...
function runDeterministicBabelRepairProof(input: {
  ...
  const filesTouched = records
    ...
    .sort();
  const finalVerifierPassed =
    verifierOutputs.length > 0 && verifierOutputs[verifierOutputs.length - 1]?.exit_code === 0;
...
function buildPromptStackEvidence(
  manifest: BenchmarkManifest,
  task: BenchmarkTask,
): {
...
function validateJsonValue(value: unknown, schema: JsonSchemaNode, path: string): string[] {
  ...
  if (schema.type !== undefined && !matchesJsonType(value, schema.type)) {
    errors.push(
      `${path}: expected type ${JSON.stringify(schema.type)}, got ${value === null ? 'null' : typeof value}`,
    );
    return errors;
  }
  if (
