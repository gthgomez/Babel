...
const SubagentPluginProofEntrySchema = z.object({
  status: z.enum(['pass', 'partial', 'fail']),
}).passthrough();

const SubagentPluginPublicProofSchema = z.object({
  ...
  public_export: z.object({
    strict: SubagentPluginProofEntrySchema,
  }).passthrough(),
}).passthrough();

const VerifierRollbackProofSchema = z.object({
  verifier_universality: z.object({
    status: z.enum(['pass', 'partial', 'fail']),
  }).passthrough(),
  hostile_rollback_corpus: z.object({
    status: z.enum(['pass', 'partial', 'fail']),
  }).passthrough(),
}).passthrough();
...
function makeGate(input: Omit<ProductionGate, 'blocking'>): ProductionGate {
  return {
    ...input,
    blocking: !input.skipped && input.required && input.status !== 'pass' && input.status !== 'skip',
  };
}
...
function proofStatusGate(input: {
  ...
  return makeGate({
    ...
    message: status === 'pass' ? input.message : `Production proof artifact is ${parsed.data.status}.`,
...
function subagentPluginPublicProofGate(input: {
  ...
  const status = missingRequirements.length === 0 ? 'pass' : ([
    readonlyStatus,
    pluginStatus,
    publicStatus,
  ].some((entry) => entry === 'fail') ? 'fail' : 'warn');
  return makeGate({
    id: input.id,
    title: input.title,
    status,
...
function verifierRollbackGates(path: string, options: { skipGateIds: Set<string> }): ProductionGate[] {
  const parsed = VerifierRollbackProofSchema.safeParse(readJsonIfExists(path));
  const isSkipped = options.skipGateIds.has('required_verifier_universality') || options.skipGateIds.has('hostile_rollback_corpus');
  if (isSkipped) {
    return [
      makeGate({
  ...
  if (!parsed.success) {
    return [
      ...
      makeGate({
        ...
        evidence_path: existsSync(path) ? path : null,
        message: `Missing or invalid verifier/rollback proof artifact: ${path}.`,
        next_action: 'Create hostile rollback proof for dirty targets, generated files, nested dirs, drift, and rollback failure.',
      }),
    ];
  }
  ...
  return [
    makeGate({
      ...
      next_action: parsed.data.verifier_universality.status === 'pass'
        ? null
    ...
    makeGate({
      ...
      next_action: parsed.data.hostile_rollback_corpus.status === 'pass'
        ? null
...
function scriptGate(input: { packageJsonPath: string; required: boolean; skipped: boolean }): ProductionGate {
  if (input.skipped) {
  ...
  return makeGate({
    ...
    next_action: matched ? null : 'Add test:live-governance:required and make it fail when DEEPSEEK_API_KEY is absent.',
  });
}
...
function buildGates(input: {
  ...
  return [
    textGate({
      ...
      requiredText: ['npm --prefix .\\babel-cli run typecheck', 'npm --prefix .\\babel-cli run build', 'npm --prefix .\\babel-cli run test:unit'],
      message: 'Core validation evidence names typecheck, build, and unit coverage.',
      nextAction: 'Refresh P8-P11 or production-proof validation evidence with typecheck, build, and unit results.',
    }),
    textGate({
      ...
      requiredText: ['validate-catalog.ps1', 'scrub checker pass/fail boundary', 'test:public-release'],
    ...
    textGate({
      ...
      requiredText: ['doctor --scope all --json` is non-red', 'benchmark product --json` is green at `27/27`'],
...
export function runProductionBenchmark(options: ProductionBenchmarkOptions = {}): ProductionBenchmarkReport {
  ...
  const proofRoot = resolve(options.proofRoot ?? join(BABEL_ROOT, 'docs', 'status', 'production-proof'));
  const statusRoot = resolve(options.statusRoot ?? join(BABEL_ROOT, 'docs', 'status'));
  const packageJsonPath = resolve(options.packageJsonPath ?? join(BABEL_ROOT, 'babel-cli', 'package.json'));
  ...
  const report: ProductionBenchmarkReport = {
    ...
    production_claim_scope: 'DeepSeek-backed governed Babel CLI/control-plane lane',
    safe_wording: summary.claim_ready
      ? 'Babel is production-ready for the proven DeepSeek-backed governed CLI lane, with provider-agnostic, market-parity, and mutating live-subagent claims excluded.'
      : 'Babel remains an experimental governance-first Prompt OS / CLI prototype with production-proof gates still open.',
    unsafe_wording: [
    ...
    next_actions: summary.claim_ready
      ...
      : gates.filter((gate) => gate.blocking).map((gate) => gate.next_action ?? `Close gate ${gate.id}.`),
  };
...
export function formatProductionBenchmarkHuman(report: ProductionBenchmarkReport): string {
  return [
    ...
    '',
    'Gates:',
    ...report.gates.map((gate) => `${gate.status.toUpperCase().padEnd(4)} ${gate.id} - ${gate.message}`),
  ].join('\n');
}
