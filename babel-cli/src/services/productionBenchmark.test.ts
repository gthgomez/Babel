...
function writeSubagentPluginPublicProof(path: string, overrides: {
  ...
  write(
    ...
    JSON.stringify({
      ...
      claim_scope: 'DeepSeek-backed governed Babel CLI/control-plane lane with live subagents excluded unless explicitly opted in',
      subagents: {
        readonly: {
      ...
      plugins: {
        ...
        remaining: 'Plugin strict proof remains partial until strict-mode distribution is recorded.',
      },
      public_export: {
        strict: {
          status: overrides.publicStatus ?? 'partial',
          evidence: [
            'tools/test-export-babel-public.ps1',
            'tools/check-public-scrub.ps1',
          ],
          notes: 'Strict public-export scrub mode should remain pass-only for warning-free exports.',
        },
        remaining: 'Legacy warning fixtures remain regression-only.',
      },
      blocking_reason: 'Subagent readonly proof, plugin strict proof, and public strict scrub proof are required before scoped production wording.',
    }, null, 2),
  );
}
...
test('production benchmark stays claim-blocked when proof artifacts are missing', () => {
  ...
  const written = JSON.parse(readFileSync(report.artifact_path, 'utf8')) as { summary?: { claim_ready?: boolean } };
...
test('production benchmark requires subagent readonly, plugin strict, and public strict proof to pass', () => {
  ...
  write(join(statusRoot, 'BABEL_P8_P11_IMPLEMENTATION_EVIDENCE_2026-06-05.md'), [
    'npm --prefix .\\babel-cli run typecheck',
    'npm --prefix .\\babel-cli run build',
    'npm --prefix .\\babel-cli run test:unit',
  ].join('\n'));
  write(join(statusRoot, 'BABEL_PUBLIC_EXPORT_PROOF_2026-06-05.md'), [
  ...
  write(join(statusRoot, 'BABEL_ROADMAP_REMAINING_PROOF_PLAN_2026-06-05.md'), [
  ...
  write(join(proofRoot, 'live-governance-breadth-proof.json'), JSON.stringify({ status: 'pass' }, null, 2));
  write(join(proofRoot, 'verifier-rollback-proof.json'), JSON.stringify({
...
test('production benchmark becomes claim-ready only with all required proof artifacts', () => {
  ...
  write(packageJsonPath, JSON.stringify({
    scripts: {
      'test:live-governance:required': 'node scripts/run_live_governance_tests.mjs --required-deepseek',
    },
  }, null, 2));
  write(join(statusRoot, 'BABEL_P8_P11_IMPLEMENTATION_EVIDENCE_2026-06-05.md'), [
    'npm --prefix .\\babel-cli run typecheck',
  ...
  assert.equal(report.skipped_gates.length, 0);
  assert.match(report.safe_wording, /DeepSeek-backed governed CLI lane/);
});
...
test('production benchmark never becomes claim-ready when a required gate is skipped', () => {
  ...
  assert.equal(report.skipped_gates.some((gate) => gate === 'subagent_plugin_public_strict'), true);
