...
test('benchmark analyzer surfaces risk labels and repair fingerprints', () => {
  ...
  writeFileSync(
    join(babelRunDir, '09_executor_gate_trace.json'),
    JSON.stringify({
      repair_state: {
        ...
        failures: [{ repeatedCount: 1 }, { repeatedCount: 2 }],
      },
    }),
    'utf8',
  );
...
test('benchmark analyzer includes stdout usage messages in repair fingerprints', () => {
  ...
  assert.match(
    analysis.work_packet.failure_fingerprint ?? '',
    /stdout=Usage: python optimized_packer/,
  );
  assert.equal(analysis.work_packet.likely_owner, 'Babel executor repair convergence');
...
test('benchmark analyzer extracts partial verifier progress from CTRF', () => {
  ...
  assert.equal(
    analysis.selected_failure?.partial_pass?.failure_categories.missing_output_artifact,
    1,
  );
