...
function makeRelicRunFixture(): { root: string; workspace: string; target: string } {
  ...
  writeFileSync(join(target, 'PROJECT_CONTEXT.md'), 'RelicRun is the active child game project.\n', 'utf-8');
...
test('ask prompt includes target-local evidence and shallow listing', () => {
...
test('streamed ask retries non-streaming after schema failure without preserving partial chunks', async () => {
  ...
  try {
    ...
    globalThis.fetch = (async (_input, init) => {
      ...
      if (calls === 1) {
        return streamingChatResponse('{"schema_version":1,"status":"ANSWER_READY","summary":"partial","answer":"Partial');
      }
