...
function scorePlanMode(projectRoot: string, cliEntry?: string): LiteFeatureScore {
  ...
  return {
    ...
    score: passed ? 1 : 0,
    detail: passed
      ? 'babel plan returned PLAN_READY on parity fixture repo.'
...
async function scoreParallelReview(): Promise<LiteFeatureScore> {
  ...
  return {
    ...
    detail: passed
      ...
      : `Parallel review harness failed: ${
          harness.scenarios
...
function scoreCheckpointUx(projectRoot: string, cliEntry?: string): LiteFeatureScore {
  ...
  const fixStatus =
    typeof fixCli.payload?.['status'] === 'string' ? fixCli.payload['status'] : null;
  const checkpoint = fixCli.payload?.['checkpoint'];
  const checkpointRecord =
    checkpoint && typeof checkpoint === 'object' ? (checkpoint as Record<string, unknown>) : null;
  const restoreCommand =
    typeof checkpointRecord?.['restore_command'] === 'string'
      ? checkpointRecord['restore_command']
      : null;
  const inspectCommand =
    typeof checkpointRecord?.['inspect_command'] === 'string'
      ? checkpointRecord['inspect_command']
      : null;
  const hasCheckpointSurface =
    restoreCommand === 'babel undo' &&
    inspectCommand !== null &&
  ...
  return {
    ...
    score: passed ? 1 : 0,
    detail: passed
      ? 'Post-fix JSON exposes babel undo and checkpoint inspect commands.'
...
function scoreVerifierDiscipline(projectRoot: string, cliEntry?: string): LiteFeatureScore {
  ...
  const cli = runBabelCli(
    ['run', '--mode', 'deep', '--json', '--project-root', projectRoot, task.task],
    {
      projectRoot,
      ...(cliEntry !== undefined ? { cliEntry } : {}),
    },
...
export async function runLiteFeatureScorecard(
  options: {
