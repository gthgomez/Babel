...
const A_MUTATE_TOOLS = new Set([
  ...
  // command-text patterns (C_GATED / D_FORBIDDEN) are evaluated first when
  // commandText is present, so force-push / credential-dump commands via bash
  // still classify c_gated / d_forbidden.
  'bash',
  'shell',
]);

/** Deterministic Class C command markers (gate actions). */
const C_GATED_COMMAND_PATTERNS: RegExp[] = [
  // Force-push / history rewrite
  /\bgit\s+push\s+(?:--force|-f)\b/i,
  /\bgit\s+reset\s+--hard\b/i,
  /\bgit\s+rebase\b/i,
  /\bgit\s+commit\s+--amend\b/i,
  /\bgit\s+clean\s+-(?:fd|df)\b/i,
  // Destructive deletion
  /\brm\s+-rf\b/i,
  // Destructive DB ops
  /\b(drop|truncate)\s+(?:table|database|schema)\b/i,
  /\bdelete\s+from\b/i,
  // Publishing / releases / deploy
  /\b(npm|pnpm|yarn)\s+publish\b/i,
  /\bgh\s+release\s+create\b/i,
  /\b(terraform\s+destroy|kubectl\s+delete|docker\s+rm\s+-f)\b/i,
  // IAM / billing / security controls
  /\b(aws|gcloud|az)\s+\w+\s+(iam|billing|organizations?)\b/i,
];

/** Deterministic Class D command markers (credential exposure — never). */
const D_FORBIDDEN_COMMAND_PATTERNS: RegExp[] = [
  /(?:cat|type|get-content|more|less|head|tail)\s+(?:[^\s]*[/\\])?\.env\b/i,
  /\b(?:cat|type|get-content|more|less|head|tail)\s+[^\s]*(?:id_rsa|id_ed25519|credentials|\.pem)\b/i,
  /\b(?:echo|printenv|env)\s+[^\s]*(?:API[_]?KEY|TOKEN|SECRET|PASSWORD)\b/i,
  /\bgit\s+log\s+--all\s+--pickaxe\b/i,
];
...
export function classifyAutonomyAction(
  ...
  if (command) {
    for (const pattern of D_FORBIDDEN_COMMAND_PATTERNS) {
      if (pattern.test(command)) return 'd_forbidden';
    }
    for (const pattern of C_GATED_COMMAND_PATTERNS) {
      if (pattern.test(command)) return 'c_gated';
    }
  }
