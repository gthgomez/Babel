...
/**
 * Deterministic (non-yielding) sibling scan. Same selection as
 * `findSiblingExamples`, used by the synchronous delivered-fragment resolver
 * so the instruction manifest can record what was actually delivered without
 * introducing an async initialization step.
 */
