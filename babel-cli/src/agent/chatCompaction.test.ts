...
function makeLongConversation(count: number): ChatMessage[] {
  const messages: ChatMessage[] = [
    {
      role: "system",
      content:
        "You are a helpful assistant with a very long system prompt that goes on for quite a while and provides lots of context to ensure the conversation is grounded.",
    },
  ];
  for (let i = 0; i < count; i++) {
    messages.push(
      {
        role: "user",
        content: `This is user message number ${i} with some additional padding to make it longer and more representative of a real conversation message.`,
      },
      {
        role: "assistant",
        content: `This is assistant response number ${i} with a detailed answer that provides useful information back to the user about their question.`,
      },
    );
  }
  return messages;
...
describe("HeuristicTruncationStrategy", () => {
  ...
  it("canApply returns false when under budget", () => {
    ...
    assert.strictEqual(
      strategy.canApply(msgs, estimated, estimated + 1000),
      false,
    );
  });
...
describe("CompactionManager", () => {
  ...
  it("skips strategies where canApply returns false", async () => {
    ...
    const result = await manager.compact(makeMessages(), {
      model: "",
      maxTokens: 100,
    });
...
describe("CompactionManager integration", () => {
  it("uses LLM compaction when available and within limits", async () => {
    ...
    const manager = new CompactionManager([
      llmStrategy,
      new HeuristicTruncationStrategy(2),
    ]);

    const msgs = makeLongConversation(5); // System + 10 messages
    const result = await manager.compact(msgs, {
      model: "test-model",
      maxTokens: 100,
    });
    ...
    assert.ok(
      ...
      `Expected ${result.length} < ${msgs.length}`,
    );
    // System prompt preserved
    assert.strictEqual(result[0]!.role, "system");
    // Should have a compaction summary
    const summaryMsgs = result.filter((m) => m.name === "compaction_summary");
...
describe("compaction preserves state", () => {
  ...
  function makeTodoConversation(): ChatMessage[] {
    return [
      ...
      {
        role: "assistant",
        content: "I will add that todo.",
        name: "tool_calls",
      },
      {
        role: "tool",
        content: 'Todo created: id=1, task="Read config file", status=pending',
        toolName: "todo_write",
      },
      ...
      {
        role: "tool",
        content: "Todo updated: id=1, status=in_progress",
        toolName: "todo_write",
      },
      ...
      {
        role: "assistant",
        content: "Completing the todo.",
        name: "tool_calls",
      },
      {
        role: "tool",
        content: "Todo updated: id=1, status=completed",
        toolName: "todo_write",
      },
  ...
  function makeWriteConversation(): ChatMessage[] {
    return [
      ...
      {
        role: "tool",
        content: "File written: /src/utils.ts (1024 bytes)",
        toolName: "write_file",
      },
      ...
      {
        role: "tool",
        content: "Text replaced: /src/index.ts (2 occurrences)",
      ...
      {
        role: "tool",
        content: "File written: /src/utils.test.ts (2048 bytes)",
        toolName: "write_file",
      },
  ...
  function makeVerifierConversation(): ChatMessage[] {
    return [
      ...
      {
        role: "tool",
        content: "Test run completed: 42 passed, 0 failed, 3 skipped",
      ...
      {
        role: "tool",
        content: "Linter finished: 0 errors, 2 warnings",
        toolName: "run_command",
      },
      ...
      {
        role: "tool",
        content: "Verifier result: all stages passed, 0 failures",
        toolName: "test_run",
      },
  ...
  it("write_file and str_replace evidence survives heuristic truncation", async () => {
    ...
    const writeResults = result.filter(
      (m) =>
        m.role === "tool" &&
        (m.toolName === "write_file" || m.toolName === "str_replace"),
    );
    assert.ok(
  ...
  it("verifier output (run_command/test_run) survives heuristic truncation", async () => {
    ...
    const verifierResults = result.filter(
      (m) =>
        m.role === "tool" &&
        (m.toolName === "run_command" || m.toolName === "test_run"),
    );
    assert.ok(
    ...
    const testRunResults = result.filter(
      (m) => m.role === "tool" && m.toolName === "test_run",
    );
