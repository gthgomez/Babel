...
describe('runUndoLane', () => {
    ...
    try {
      withoutLatestRunPointer(() => {
        assert.throws(
          ...
          /No recoverable run found for babel undo/,
        );
      });
    } finally {
  ...
  it(
    'restores from the latest small-fix checkpoint and writes undo artifacts under runs/babel-lite',
    { concurrency: false },
    async () => {
      ...
      try {
        ...
        assert.equal(result.payload.checkpoint.restore_command, 'babel undo');
        assert.ok(result.payload.run_dir?.replace(/\\/g, '/').includes('runs/babel-lite'));
        assert.equal(
          readFileSync(join(repo, 'src', 'math.js'), 'utf-8'),
          'export const add = () => 0;\n',
        );
        assert.match(result.humanText, /Rollback: babel undo/);
      } finally {
  ...
  it(
    'prefers the requested project root over a newer global latest pointer',
    { concurrency: false },
    async () => {
      ...
      try {
        ...
        assert.equal(
          readFileSync(join(repoA, 'src', 'math.js'), 'utf-8'),
          'export const add = () => 0;\n',
        );
        assert.equal(
          readFileSync(join(repoB, 'src', 'math.js'), 'utf-8'),
