...
export class AgentProgressPane extends Component {
  ...
  override render(): string {
    ...
    const nameLine = sublabel
      ? `${bold(accentHigh(name))} ${dim(`(${sublabel})`)}`
      : bold(accentHigh(name));
  ...
  renderCompact(): string {
    ...
    const indicator = this.statusIndicator(status ?? "pending");
    const costStr =
      cost !== undefined && cost > 0 ? ` ${dim(`$${cost.toFixed(3)}`)}` : "";
...
export class AgentTeamOverview extends Component {
  ...
  override render(): string {
    ...
    const statuses = [...this.agents.values()].map(
      (a) => (a as any).info?.status ?? "pending",
    );
    ...
    const progressBar =
      success("█".repeat(filled)) +
      meterTrack("░".repeat(progressWidth - filled));
