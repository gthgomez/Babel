...
export function renderRunPrelude(context: RunPreludeContext): string {
  ...
  const statusRows = renderStatusRows([
    ...
    { label: "Tier", value: context.tier ?? "policy default" },
    {
      ...
      value: context.executionProfile ?? "safe_repo",
    },
  ...
  blocks.push(
    renderSection("PIPELINE", [renderStageTimeline(stages)], "layered flow"),
  );
  return blocks.join("\n");
}
...
export function renderRoutingSummary(manifest: RoutingManifest): string {
  const rows = renderStatusRows([
    ...
    {
      label: "Domain",
      value: manifest.instruction_stack?.domain_id ?? "n/a",
      tone: "accent",
    },
    {
      label: "Adapter",
      value: manifest.instruction_stack?.model_adapter_id ?? "n/a",
    },
    {
      label: "Stages",
      value:
        (manifest.instruction_stack?.pipeline_stage_ids ?? []).join(", ") ||
        "(none)",
    },
    {
      label: "Manifest",
      value: `${manifest.prompt_manifest?.length ?? 0} prompt file(s)`,
    },
  ]);
...
export function renderPlanSummary(plan: PlanData): string {
  ...
  const blocks: string[] = [
    renderStatusRows([
      {
        label: "Plan Type",
        value: plan.plan_type ?? "IMPLEMENTATION_PLAN",
        tone: "accent",
      },
...
export function renderQaSummary(verdict: QaVerdict): string {
  const failures =
    verdict.failures
      ?.slice(0, 5)
      .map((failure) => `[${failure.tag}] ${failure.condition}`) ?? [];
  const blocks: string[] = [
    renderStatusRows([
      {
        label: "Verdict",
      ...
      {
        label: "Confidence",
        value: `${verdict.overall_confidence ?? "n/a"}/5`,
      },
  ...
  return renderSection(
    "QA",
    blocks,
    verdict.verdict === "PASS" ? "gate passed" : "gate rejected",
  );
}
  ...
  return renderSection(
    ...
    [
      renderStatusRows([
        ...
        {
          label: "Mode",
          value: isDryRun ? "dry-run executor" : "live executor",
          tone: "accent",
        },
  ...
  return renderSection(
    ...
    [
      ...
      renderStatusRows([
        {
          label: "Prompt",
          value: formatInteger(usageSummary.totalInputTokens),
        },
        {
          label: "Completion",
          value: formatInteger(usageSummary.totalOutputTokens),
        },
        {
          label: "Models",
          value: String(Object.keys(usageSummary.modelBreakdown ?? {}).length),
        },
      ]),
    ],
    "final state",
...
export function renderDoctorSummary(
  ...
  for (const section of sectionOrder) {
    const sectionChecks = result.checks.filter(
      (check) => check.section === section,
    );
    if (sectionChecks.length === 0) continue;

    lines.push(
      renderSectionHeader(section, `${sectionChecks.length} check(s)`),
    );
    lines.push(
      renderCheckRows(
...
export function renderResumeSummary(
  title: string,
  rows: StatusRow[],
  metadata = "",
): string {
...
export function renderDryRunSummary(state: DryRunState): string {
  ...
  const modeDetail = state.effective
    ? "dry-run executor"
    : "live executor via sandbox";
...
export function renderOperatorHeader(state: Record<string, unknown>): string {
  ...
  const badgeStatus =
    userStatus === "ready" ? "READY" : userStatus.toUpperCase();
  const statusBadge = renderBadge(badgeStatus);
  const project = state.project
    ? accentBright(state.project as string)
    : muted("global");
  ...
  const mode = state.mode
    ? accentBright((state.mode as string).toUpperCase())
    : muted("CHAT");
  const turnInfo =
