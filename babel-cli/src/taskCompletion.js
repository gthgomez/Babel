...
export function isDeliverableCompletenessReject(verdict) {
  return Boolean(
    verdict?.failures?.some((failure) =>
      String(failure?.condition ?? '').includes('[DELIVERABLE_COMPLETENESS]'),
    ),
  );
}
...
export function reconcileQaVerdictWithGroundedEvidence(verdict, registry) {
  ...
  return {
    ...
    ...(verdict.proposed_fix_strategy
      ? { proposed_fix_strategy: verdict.proposed_fix_strategy }
      : {}),
  };
}
