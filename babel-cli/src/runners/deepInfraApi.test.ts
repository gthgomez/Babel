...
const originalStreamIdle =
  process.env["BABEL_DEEPINFRA_STREAM_IDLE_TIMEOUT_MS"];
...
test("DeepInfra API runner retries transport aborts before failing the call", async () => {
  ...
  const result = await new DeepInfraApiRunner(
...
test("DeepInfra API runner emits one receipt when a streamed response ends without terminal marker", async () => {
  process.env["DEEPINFRA_API_KEY"] = "test-key";
  const { DeepInfraApiRunner } = await import("./deepInfraApi.js");
  ...
  await assert.rejects(
    () =>
      new DeepInfraApiRunner("deepseek-ai/DeepSeek-V3-0324").executeRaw(
        "say hello",
        {
          onChunk: () => {},
  ...
  assert.equal(failures[0]!.normalized_failure_class, "INCOMPLETE_STREAM");
  assert.equal(failures[0]!.partial_model_output, true);
});

test("DeepInfra native tool stream emits a receipt before returning an incomplete-stream error", async () => {
  process.env["DEEPINFRA_API_KEY"] = "test-key";
  const { DeepInfraApiRunner } = await import("./deepInfraApi.js");
  ...
  const events: any[] = [];
  const failures: any[] = [];
  for await (const event of new DeepInfraApiRunner(
    "deepseek-ai/DeepSeek-V3-0324",
...
test("DeepInfra API runner executeWithToolsStream yields error on HTTP failure", async () => {
  ...
  assert.equal(failures[0]!.http_status, 401);
