...
const MAX_TOKENS =
  Number.isFinite(_rawTokens) && _rawTokens > 0 ? _rawTokens : 32000;
const _rawRequestTimeoutMs = Number(
  process.env["BABEL_DEEPINFRA_REQUEST_TIMEOUT_MS"] ?? "120000",
);
const REQUEST_TIMEOUT_MS =
  Number.isFinite(_rawRequestTimeoutMs) && _rawRequestTimeoutMs > 0
    ? _rawRequestTimeoutMs
    : 120000;
const _rawRequestMaxRetries = Number(
  process.env["BABEL_DEEPINFRA_REQUEST_MAX_RETRIES"] ?? "4",
);
...
interface OpenRouterResponseMetadata {
  endpoints?: {
    available?: Array<{
      provider?: string;
      model?: string;
...
function upstreamProviderFromResponse(value: {
  ...
  const selected = value.openrouter_metadata?.endpoints?.available?.find(
    (endpoint) =>
      endpoint.selected === true && typeof endpoint.provider === "string",
  );
...
function getStreamMaxRetries(): number {
  const parsed = Number(
    process.env["BABEL_DEEPINFRA_STREAM_MAX_RETRIES"] ?? "",
  );
  const value =
...
function mapProviderMessages(
  ...
  return mapProviderMessagesToWire(
    messages,
    defaultSystemPrompt,
...
async function readStreamingResponse(
  ...
  while (true) {
    ...
    const idle = new Promise<never>((_, reject) => {
      timeout = setTimeout(() => {
        ...
        reject(
          new Error(
            `[deepInfraApi] stream idle timeout after ${idleTimeoutMs}ms`,
          ),
        );
      }, idleTimeoutMs);
    });
  ...
  if (!terminalMarkerSeen) {
    throw new Error(
      "[deepInfraApi] incomplete stream: terminal marker was not received",
    );
  }
...
export class DeepInfraApiRunner implements LlmRunner {
  ...
  constructor(
    ...
    this.apiKey =
      ...
      resolveProviderCredential(provider, {
    ...
    this.model = model;
    this.maxTokens =
      typeof sampling.maxTokens === "number" &&
      Number.isFinite(sampling.maxTokens) &&
  ...
  private async _executeRequest(
    ...
    let lastObservedBody: ChatResponse | null = null;
    ...
    for (let attempt = 1; attempt <= requestMaxRetries; attempt += 1) {
      ...
      try {
        ...
        notifyPhase(
          "provider_error",
          undefined,
          isAbortError(err) ? "timeout" : "transport",
        );
    ...
    if (!response) {
      ...
      throw (
        lastError ??
        new Error(
          `[deepInfraApi] request failed before receiving a response (${this.model})`,
        )
      );
    }
    ...
    if (!response.ok) {
      ...
      notifyCompleted("failed", null, body);
        ...
        try {
          ...
          if (
            ...
            notifyPhase(
              "provider_error",
              undefined,
              isAbortError(error) ? "timeout" : "stream",
            );
            lastFailureStage = "stream";
            lastFailureMessage = streamOutputEmitted
              ? "incomplete stream after partial model output; retry suppressed"
            ...
            notifyCompleted(
              "failed",
              streamState.observedModelId,
              streamOutputEmitted ? "[partial model output]" : text,
              streamState.upstreamProvider,
            );
            throw error;
          }
          ...
          if (signal) {
            ...
            signal.addEventListener("abort", onStreamRetryAbort, {
              once: true,
            });
          }
  ...
  async execute<T>(
    ...
    this.lastInvocationMetadata = buildInvocationMetadata(
        ...
        : this.lastInvocationMetadata?.total_tokens
          ? {
              ...
              this.lastInvocationMetadata.completion_tokens !== undefined
                ? {
                    completion_tokens:
                      this.lastInvocationMetadata.completion_tokens,
                  }
                : {}),
      ...
      validationMs,
      streamState.observedModelId ??
        this.lastInvocationMetadata?.observed_model_id ??
        null,
      streamState.upstreamProvider ??
        this.lastInvocationMetadata?.upstream_provider ??
        null,
    );
  ...
  async *executeWithToolsStream(
      ...
      if (status === "failed" && !failureNotified) {
        ...
        callbacks?.onProviderFailure?.(
          buildProviderFailureReceipt({
            ...
            localRequestId: inferenceId,
            upstreamProvider,
            openrouterRequestId: providerRequestId(lastResponse),
            httpStatus: lastHttpStatus,
            apiErrorCode: lastApiErrorCode,
            normalizedFailureClass: failureClass,
            message: lastFailureMessage,
    ...
    if (!response) {
      ...
      yield {
        type: "error",
        message: lastError?.message ?? "[deepInfraApi] No response received",
      };
      return;
    }
    ...
    if (!response.ok) {
      ...
      yield {
        type: "error",
        message: `[deepInfraApi] HTTP ${response.status} (${this.model}): ${body}`,
      };
      return;
    }
    ...
    if (!response.body) {
      ...
      yield {
        type: "error",
        message: "[deepInfraApi] Streaming response had no body.",
      };
      return;
    }
    ...
    try {
      ...
      while (true) {
        ...
        const idle = new Promise<never>((_, reject) => {
          readTimeout = setTimeout(() => {
            ...
            reject(
              new Error(
                `[deepInfraApi] stream idle timeout after ${streamIdleTimeoutMs}ms`,
              ),
            );
          }, streamIdleTimeoutMs);
        });
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (upstreamProvider)
              streamState.upstreamProvider = upstreamProvider;
      ...
      if (!terminalMarkerSeen) {
        ...
        yield {
          type: "error",
          message: `[deepInfraApi] ${lastFailureMessage}`,
        };
        return;
      }
    ...
    notifyCompleted(
      invocationFailed ? "failed" : "delivered",
      streamState.observedModelId,
      outputReceipt,
      streamState.upstreamProvider,
    );
