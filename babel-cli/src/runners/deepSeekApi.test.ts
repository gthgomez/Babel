...
test("DeepSeek API runner emits a hashed provider receipt for terminal HTTP failures", async () => {
  process.env["DEEPSEEK_API_KEY"] = "sk-test-key";
  const { DeepSeekApiRunner } = await import("./deepSeekApi.js");
  globalThis.fetch = (async () =>
  ...
  assert.equal(failures[0]!.normalized_failure_class, "HTTP_4XX_OTHER");
  assert.equal(failures[0]!.api_error_code, "invalid_request");
...
test("DeepSeek API runner requires DEEPSEEK_API_KEY", async () => {
  ...
  assert.throws(
    () => new DeepSeekApiRunner("deepseek-v4-flash"),
...
test("DeepSeek native POST retains the durable committed compaction capsule", async () => {
  ...
  appendThreadEvent(log, {
    ...
    turn_id: turnId,
    content: "COMMITTED CAPSULE: preserve this repair context",
  ...
  assert.equal(
    postedMessages.filter((message) => message.role === "system").length,
    1,
  );
  assert.match(
    postedMessages[0]!.content,
    /COMMITTED CAPSULE: preserve this repair context/,
  );
  ...
  assert.deepEqual(scheduled, [
    {
      ...
      attempt: 2,
      reason: "server_error",
      backoff_ms: (scheduled[0] as any).backoff_ms,
    },
  ]);
...
test("DeepSeek settles each retry before scheduling the next failed attempt", async () => {
  ...
  assert.deepEqual(
    settled.map((event) => [event.attempt, event.outcome]),
    [
      [2, "failed"],
      [3, "succeeded"],
    ],
...
test("DeepSeek native tool stream emits a complete retry lifecycle", async () => {
  ...
  const events: string[] = [];
  for await (const event of new DeepSeekApiRunner("deepseek-v4-flash")
