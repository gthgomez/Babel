...
| Portable workflow contract (non-normative, subordinate to harness-v1) | `docs/guides/PORTABLE_WORKFLOW_CONTRACT.md` |
...
4. **Run the File Size Ratchet check before pushing** when you touched large files: `pwsh tools/preflight-ratchet.ps1` (or `pwsh tools/check-architectural-budget.ps1`). This is currently enforced locally only — it is not wired into CI (see `handoffs/budget-recovery-design.md` for the planned enforcement).
