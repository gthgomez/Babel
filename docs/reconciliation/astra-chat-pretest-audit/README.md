...
# Astra Chat pre-experiment audit — evidence and implementation packet
...
inference. [PROVENANCE.md](PROVENANCE.md) records that the clone attempt failed on
DNS resolution and that the raw external-session and log artifacts are withheld
from this publication. This packet is not a runtime certification and
does not claim exhaustive inspection of every optional provider, MCP server,
permission profile, or recovery path.
...
[probe_results.json](probe_results.json) records 16 probe cases containing 27
assertions (all 16 cases satisfied, 0 failed) on Node v24.21.0. **These are
assertions about suspected defects and
positive controls, not sixteen readiness passes.** P01–P11 use manually
transcribed or adapted source logic / individual expressions and explicit
fixtures. P12–P16 are smaller control-flow models, not the actual SSE parser or
stream adapter. All probes are offline, with zero network calls and zero
inference calls. The transcriptions are not certified byte-identical copies of
the repository; the integrated replacements below must test the actual
production modules.

[extracted_logic_probes.mjs](extracted_logic_probes.mjs) run with
`--out probe_results.json` rewrites that evidence file; `node
extracted_logic_probes.mjs` alone prints results to stdout without modifying
tracked files.
...
an injection-path difference; it does not establish that every upstream TUI caller
...
Require `BABEL_RUNTIME_INVARIANTS=enforce` in the experiment preflight, and
demonstrate that invalid payloads generate no outbound request.
...
6. **Subagent/process fixture.** Test zero-change, no-finish round exhaustion,
   ...
   concurrently. On Windows and Linux, spawn a descendant process and demonstrate
   no post-terminal writes. Exercise worktree setup, merge conflicts, and cleanup.
