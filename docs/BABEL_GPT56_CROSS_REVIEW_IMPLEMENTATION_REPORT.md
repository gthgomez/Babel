...
**Status**: IMPLEMENTATION REPORT — uncommitted (per §31: no commits without explicit policy authorization; diff left in the working tree).
**Date**: 2026-08-15
...
| P1-H | Config-drift baselines via the existing verifierIntegrity machinery (`computeConfigBaselineHashes` / `detectConfigDrift`), bounded default path list; drift is evidence, never auto-revert. | `verifierIntegrity.ts` |
...
| E2 evidence envelope | `READY` — `buildEvidenceEnvelope` shipped as seam (env-gated wiring not yet default) |
| E3 compaction calibration | `READY` — no code needed; config knob exists |
| E4 reasoning effort | `READY` — `mapReasoningEffort` + capability records; DeepSeek resolves `unsupported` honestly |
| E5 routing ledger | `READY` — telemetry fields shipped; join still needs live cell data |
...
EXPERIMENTS_READY: E1 READY · E2 READY (seam) · E3 READY · E4 READY · E5 READY · E7-A READY ·
  E7-B NOT_READY (provider availability) · E6 BLOCKED_REQUIRES_APPROVAL (live OpenAI adapter + spend).
...
NEXT_RECOMMENDED_ACTION: 1) stage/commit per operator policy; 2) run CI-gate suites
  ...
  §14 cross-review protocol; 5) E6 after live-OpenAI approval.
```
