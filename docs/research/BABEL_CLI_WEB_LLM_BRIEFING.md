...
(`example_saas_backend`, `example_llm_router`, `example_web_audit`, `example_mobile_app`). Its job is to assemble the **smallest correct
...
├── 00_System_Router/            ← OLS-v9-Orchestrator.md (default, reliable, current runtime)
├── 01_Behavioral_OS/            ← Cognitive rules that apply to every agent
├── 02_Domain_Architects/        ← Thin strategy shells (Backend, Frontend, Android, Compliance, …)
...
| 5 | `05_Project_Overlays` | Repository-specific constraints (hard invariants for example_saas_backend, etc.) |
...
Tier 3 → Claude CLI     (high-judgment, safeguarded quality)
```
...
`OLS-v8-Orchestrator.md` remains historical for migration reference and is not part of the live runtime contract.
...
```
Options:
  --project <name>     Target project (example_saas_backend | example_llm_router | example_web_audit | example_mobile_app)
...
Alias for `babel run --mode plan --project <project> <intent>`.
...
| `audit_ui` | No | Logs intent | Spawn the example_web_audit orchestrator against a URL |
...
2. **`OLS-v8-Orchestrator.md` JSON contract** — historical reference only. The live runtime uses only the v9 contract; v8 is retired.
...
| Change orchestrator behavior | `00_System_Router/OLS-v9-Orchestrator.md` |
