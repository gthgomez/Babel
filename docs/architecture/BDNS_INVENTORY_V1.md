...
| `SessionEvent` `mutation_batch` | The canonical caller's declared mutation receipt and status | It captures declared intent and receipt publication, not every filesystem mutation |
| File writes in `agent/codingLoop`, `agent/toolExecutor`, and services | Specific mutation paths at explicit call sites | A successful write call alone cannot establish durable on-disk state |
