...
Each touch point is classified exactly one of:
...
Any recurring manual task is presumed `AUTOMATION_DEBT` unless a documented
authority or safety reason proves otherwise.
...
| T2 | Worktree/branch creation | `scripts/agent-worktree.ps1` (`create`/`list`) | `AUTOMATION_DEBT` (agent-side) | per task | No owner touch. No lease/registration semantics; collisions surface only as path-exists errors; no stale detection. |
...
| T9 | Merge of a gate-green ordinary PR | required checks green at exact head + threads resolved + review evidence valid; merge is then a normal `gh pr merge` (ruleset requires 0 approvals, no bypass actors) | `AUTOMATION_DEBT` → reclassified as bounded-authorized by the [rule 05 amendment](../../.agents/rules/05-github-workflow.md) in this change set | per PR | Merge authority for Tier 0–2 is policy-authorized; Tier 3 additionally requires the frontier review record; trust-root merges stay `OWNER_REQUIRED`. |
...
| T11 | Branch/worktree cleanup after merge | previously blocked by the blanket no-delete rule; now covered by the bounded cleanup exception | `AUTOMATION_DEBT` (was `UNNECESSARY_PERMISSION_GATE`) | per PR | Stale branches/worktrees accumulated under the old rule (7 stale remote `agent/*` branches observed 2026-09-05). |
