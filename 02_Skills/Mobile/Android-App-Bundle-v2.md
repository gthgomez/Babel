...
# Skill: Android App Bundle Packaging (v2.0)
...
- This skill covers AAB packaging and store artifact selection. It does not cover store listing content, Data Safety declarations, or pricing — those are handled by store-specific skills.
- This skill codifies Babel's multi-store conventions. It does not replace bundletool or Android developer documentation.

## Failure Behavior of This Skill
- **Flavor produces wrong billing implementation:** Verify via bundletool APK extraction. Halt release if mismatch detected.
- **Store rejects AAB due to unsupported dynamic feature:** Remove the feature from that store's flavor, rebuild, re-validate.
- **bundletool unavailable:** Manual APK extraction via Android Studio. Log the manual step explicitly.

## Strategic Next Move
After every packaging plan, end with one next-move question: confirm the exact flavor and upload target for each store.

## References
- `skill_android_release_build` — signing config, ProGuard, 16KB compliance
- `skill_google_play_store`, `skill_amazon_appstore`, `skill_samsung_galaxy_store` — store-specific policies
- `ols-compiler` (`04_Meta_Tools/OLS-MCC/ols-compiler/SKILL.md`) — for hardening packaging workflows
