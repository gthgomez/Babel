...
# Skill: Multi-Agent Pipeline (v2.0)
...
- This skill governs multi-agent pipeline architecture (agent base classes, timeout chains, aggregation, learning pipelines). It does not govern individual agent state transitions (see `skill_autonomous_agent_state_machine`), file-level concurrency (see `skill_workspace_locking`), or structured result delivery (see `skill_async_task_delivery`).
- Pipeline guardrails (feature flags, confidence thresholds) must not be bypassed. Learning auto-apply must remain gated behind explicit approval.

## Failure Behavior of This Skill
- **Pipeline produces contradictory agent outputs:** Halt aggregation. Flag as CONFLICT. Escalate to human review — do not silently merge conflicting results.
- **Learning pipeline auto-applies without approval:** This is a CRITICAL safety violation. The `learning_auto_apply` flag must remain `false` in production. Revert any auto-applied change immediately.
- **Agent timeout cascade:** If multiple agents timeout in the same run, reduce parallelism or increase per-agent timeout budgets. Do not silently accept partial results.

## Strategic Next Move
After every pipeline plan, end with one next-move question: confirm the learning guardrails are enabled and the aggregation weights are calibrated.

## References
- `skill_workspace_locking` (`02_Skills/Governance/Workspace-Locking-v2.md`) — file-level concurrency for parallel agents.
- `skill_autonomous_agent_state_machine` (`02_Skills/Governance/Autonomous-Agent-State-Machine-v2.md`) — per-agent state transitions during pipeline execution.
- `skill_async_task_delivery` (`02_Skills/Governance/Async-Task-Delivery-v2.md`) — structured result delivery from pipeline agents.
- `skill_parallel_swarm_governance` (`02_Skills/Governance/Parallel-Swarm-Governance-v2.md`) — swarm dispatch and sector isolation.
- `Workflow-Patterns/Hierarchical-Delegation-v1.md` — composable parent→sub-agents→aggregate pattern.
- `ols-compiler` (`04_Meta_Tools/OLS-MCC/ols-compiler/SKILL.md`) — for hardening pipeline contracts.
- `coherence-linter` (`04_Meta_Tools/coherence-linter/SKILL.md`) — for detecting contradictions across multi-agent governance skills.
