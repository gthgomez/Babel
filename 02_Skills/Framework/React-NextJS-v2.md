...
- This skill provides Babel-specific Next.js + React conventions. It does not replace Next.js or React official documentation.
- Version-specific guidance (App Router, Server Components, React 19 features) must be verified against current stable releases before use in production plans.

## Failure Behavior of This Skill
- **Referenced Next.js/React version is outdated:** Flag as STALE. Recommend web-search verification against current docs.
- **Guidance conflicts with another skill:** Activate `coherence-linter` to detect and resolve.
- **Pattern conflicts with `skill_react_vite`:** These skills are declared as catalog conflicts. Do not load both for the same task.
...
## References
- `ols-compiler` (`04_Meta_Tools/OLS-MCC/ols-compiler/SKILL.md`) — for hardening frontend patterns.
- `skill_standards_currency_audit` (`02_Skills/Governance/Standards-Currency-Audit-v2.md`) — for scheduled re-verification of Next.js version pins.
- `coherence-linter` (`04_Meta_Tools/coherence-linter/SKILL.md`) — for detecting contradictions with related frontend skills.
