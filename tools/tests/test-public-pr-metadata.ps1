...
try {
  ...
  Assert-True ($checkerSource -match 'ResponseHeadersVariable responseHeaders') 'checker must capture response headers portably'
  Assert-True ($checkerSource -match "responseHeaders\['Link'\]") 'checker must tolerate commit responses without a Link header under strict mode'
