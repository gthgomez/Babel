...
function Remove-FixtureWorktree {
  param([Parameter(Mandatory = $true)][string]$Path, [string]$Repo = $fixture)
  if (Test-Path -LiteralPath $Path -PathType Container) {
    Invoke-TestGit @('-C', $Repo, 'worktree', 'remove', '--', $Path) | Out-Null
  }
}
...
try {
  ...
  $fallbackExpected = Join-Path (Join-Path $publicRoot 'worktrees') (Join-Path 'Babel' 'fallback')
  ...
  # A clone whose directory name is not a safe path segment (for example a
  # space) must still work whenever the clone directory name is not embedded
  # in the resolved root (explicit override, or the clone-local fallback).
  $spaceClone = Join-Path $publicRoot 'Babel space clone'
  New-Item -ItemType Directory -Path $spaceClone -Force | Out-Null
  Invoke-TestGit @('-C', $spaceClone, 'init', '--initial-branch=main') | Out-Null
  Invoke-TestGit @('-C', $spaceClone, 'config', 'user.name', 'Babel Worktree Storage Test') | Out-Null
  Invoke-TestGit @('-C', $spaceClone, 'config', 'user.email', 'babel-worktree-storage@example.invalid') | Out-Null
  Set-Content -LiteralPath (Join-Path $spaceClone 'README.md') -Value '# fixture' -Encoding utf8
  Invoke-TestGit @('-C', $spaceClone, 'add', 'README.md') | Out-Null
  Invoke-TestGit @('-C', $spaceClone, 'commit', '-m', 'fixture base') | Out-Null
  Invoke-TestGit @('-C', $spaceClone, 'remote', 'add', 'origin', 'https://github.com/gthgomez/Babel.git') | Out-Null
  $spaceRoot = Join-Path $fixtureRoot 'space-worktrees'
  $spaced = Invoke-Helper @('-Action', 'create', '-RepoRoot', $spaceClone, '-GitPath', $git, '-ExpectedRepository', 'gthgomez/Babel', '-StorageConfig', $config, '-WorktreeRoot', $spaceRoot, '-Base', 'HEAD', '-Name', 'spaced', '-NoFetch')
  Assert-CentralStorageTest ($spaced.exitCode -eq 0) "explicit WorktreeRoot for a spaced clone name failed: $($spaced.text)"
  $spacedRecord = $spaced.text | ConvertFrom-Json
  Assert-CentralStorageTest ([IO.Path]::GetFullPath($spacedRecord.path) -eq [IO.Path]::GetFullPath((Join-Path $spaceRoot 'spaced'))) 'spaced-clone explicit WorktreeRoot was not honored'
  Remove-FixtureWorktree -Path $spacedRecord.path -Repo $spaceClone
