...
$kickoffPrompt = if ($compactKickoffActive) {
    if ($repoLocalSystemPresent) {
        "Read C:\Workspace\[REDACTED_PRIVATE_IDENTIFIER]\BABEL_BIBLE.md, then this repo's PROJECT_CONTEXT.md and LLM_COLLABORATION_SYSTEM before planning or coding."
    } elseif ($repoContextFiles.Count -gt 0) {
        "Read C:\Workspace\[REDACTED_PRIVATE_IDENTIFIER]\BABEL_BIBLE.md, then this repo's PROJECT_CONTEXT.md before planning or coding."
    } else {
        "Read C:\Workspace\[REDACTED_PRIVATE_IDENTIFIER]\BABEL_BIBLE.md before planning or coding."
    }
} elseif ($repoLocalSystemPresent) {
    "Read Babel's C:\Workspace\[REDACTED_PRIVATE_IDENTIFIER]\BABEL_BIBLE.md first, use Babel to select the right instruction stack for this task, then read this repo's PROJECT_CONTEXT.md and LLM_COLLABORATION_SYSTEM/README_FOR_HUMANS_AND_LLMS.md before planning or coding."
} elseif ($repoContextFiles.Count -gt 0) {
    "Read Babel's C:\Workspace\[REDACTED_PRIVATE_IDENTIFIER]\BABEL_BIBLE.md first, use Babel to select the right instruction stack for this task, then read this repo's PROJECT_CONTEXT.md before planning or coding."
} else {
    "Read Babel's C:\Workspace\[REDACTED_PRIVATE_IDENTIFIER]\BABEL_BIBLE.md first and use Babel to select the right instruction stack for this task before planning or coding."
}
