...
if (!receiptPath || !keysPath || !ledgerPath || !supervisorKeysPath) {
  console.log(
    JSON.stringify({
      valid: false,
      errors: ["receipt_key_registry_or_ledger_or_supervisor_registry_missing"],
    }),
  );
  process.exit(1);
}
...
try {
  supervisorRegistry = JSON.parse(readFileSync(supervisorKeysPath, "utf8"));
} catch {
  errors.push("supervisor_key_registry_malformed");
}
...
for (const record of ledger.challenges) {
  ...
  const supervisorPem =
    supervisorRegistry.schema_version === 1 &&
    supervisorRegistry.keys &&
    typeof supervisorRegistry.keys[record.supervisor_signature.key_id] === "string"
      ? supervisorRegistry.keys[record.supervisor_signature.key_id]
      : undefined;
  if (
    !supervisorPem ||
    ...
    try {
      const valid = verify(
        null,
        Buffer.from(JSON.stringify(canonicalize(unsignedRecord)), "utf8"),
        createPublicKey(supervisorPem),
      ...
      errors.push("review_challenge_supervisor_signature_invalid");
    }
  }
}
