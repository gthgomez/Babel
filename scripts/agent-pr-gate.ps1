...
function Get-AgentRulesetPolicy {
  ...
  $rules = @(Get-AgentLocalValue -Object $detail -Name 'rules')
  $pullRules = @($rules | Where-Object { [string]$_.type -eq 'pull_request' })
  ...
  $pullParameters = Get-AgentLocalValue -Object $pullRule -Name 'parameters'
  $statusParameters = Get-AgentLocalValue -Object $statusRule -Name 'parameters'
  if ($null -eq $pullParameters -or $null -eq $statusParameters) { return [pscustomobject]@{ available = $false; error = 'active_ruleset_parameters_missing'; id = [string]$candidate.id } }
  $requiredStatusEntries = @(Get-AgentLocalValue -Object $statusParameters -Name 'required_status_checks')
  return [pscustomobject][ordered]@{
    available = $true; id = [int64]$detail.id; name = [string]$detail.name; enforcement = [string]$detail.enforcement
    required_approving_review_count = [int](Get-AgentLocalValue -Object $pullParameters -Name 'required_approving_review_count')
    required_review_thread_resolution = [bool](Get-AgentLocalValue -Object $pullParameters -Name 'required_review_thread_resolution')
    require_code_owner_review = [bool](Get-AgentLocalValue -Object $pullParameters -Name 'require_code_owner_review')
    allowed_merge_methods = @((Get-AgentLocalValue -Object $pullParameters -Name 'allowed_merge_methods') | ForEach-Object { [string]$_ })
    strict_required_status_checks_policy = [bool](Get-AgentLocalValue -Object $statusParameters -Name 'strict_required_status_checks_policy')
    required_status_checks = @($requiredStatusEntries | ForEach-Object { [string](Get-AgentLocalValue -Object $_ -Name 'context') })
...
function Read-AgentIndependentReceipt {
  ...
  try {
    ...
    foreach ($temporary in @($keyPath, $supervisorKeyPath, $verifierPath)) { if (Test-Path -LiteralPath $temporary -PathType Leaf) { Remove-Item -LiteralPath $temporary -Force -ErrorAction SilentlyContinue } }
  }
...
try {
  ...
  $detachedCandidate = Test-AgentIntentionalDetachedCandidate -IsDetached ([string]::IsNullOrWhiteSpace($localBranch)) -AllowIntentionalDetachedCandidate ([bool]$AllowIntentionalDetachedCandidate) -RequireIsolatedWorktree ([bool]$RequireIsolatedWorktree) -LocalHead ([string]$localHead) -ReviewedHead ([string]$ReviewedHeadSha)
  $detachedCandidateAuthorized = [bool]$AllowIntentionalDetachedCandidate -and [bool]$detachedCandidate.accepted
  Add-AgentCheck -Name 'INTENTIONAL_DETACHED_CANDIDATE' -Passed ($detachedCandidateAuthorized -or -not $AllowIntentionalDetachedCandidate) -Blocker 'detached_candidate_not_exact_or_isolated'
  Add-AgentCheck -Name 'ON_BRANCH' -Passed ((-not [string]::IsNullOrWhiteSpace($localBranch)) -or $detachedCandidateAuthorized) -Blocker 'detached_head'
  # Worktree cleanliness is read through the shared status --porcelain=v2
  # parser so staged, unstaged, untracked, and detached states are explicit.
  ...
  if ($ghAvailable -and $authOk) {
    ...
    Add-AgentCheck -Name 'EXPECTED_REPO' -Passed ($null -ne $repoView -and [string]::Equals([string](Get-AgentProperty -Object $repoView -Name 'nameWithOwner' -Default ''), $ExpectedRepository, [StringComparison]::OrdinalIgnoreCase)) -Blocker 'github_repository_metadata_mismatch'
  ...
  $prHead = if ($prAvailable) { [string](Get-AgentProperty -Object $prView -Name 'headRefOid' -Default '') } else { '' }
  $prBase = if ($prAvailable) { [string](Get-AgentProperty -Object $prView -Name 'baseRefOid' -Default '') } else { '' }
  $prHeadBranch = if ($prAvailable) { [string](Get-AgentProperty -Object $prView -Name 'headRefName' -Default '') } else { '' }
  $prBaseBranch = if ($prAvailable) { [string](Get-AgentProperty -Object $prView -Name 'baseRefName' -Default '') } else { '' }
  Add-AgentCheck -Name 'REVIEWED_HEAD_INPUT' -Passed (Test-AgentShaValue $ReviewedHeadSha) -Blocker 'reviewed_head_sha_required'
  ...
  $mergeState = if ($prAvailable) { [string](Get-AgentLocalValue -Object $prView -Name 'mergeStateStatus') } else { '' }
  $mergeable = if ($prAvailable) { [string](Get-AgentLocalValue -Object $prView -Name 'mergeable') } else { '' }
  $mergeStateResolution = Resolve-AgentTrustedMergeState -RunningTrustedSelfCheck $runningTrustedControlPlane -MergeStateStatus $mergeState -Mergeable $mergeable
  $selfCheckPending = $mergeStateResolution.reason -eq 'trusted_self_check_pending_is_deferred'
  Add-AgentCheck -Name 'MERGE_STATE_CLEAN' -Passed ($prAvailable -and [bool]$mergeStateResolution.accepted) -Blocker 'pr_merge_state_not_clean'
  Add-AgentCheck -Name 'MERGE_STATE_SELF_DEPENDENCY' -Passed (-not $runningTrustedControlPlane -or $selfCheckPending -or [string]::Equals($mergeState, 'CLEAN', [StringComparison]::OrdinalIgnoreCase)) -Blocker 'trusted_self_check_merge_state_unresolved'
  ...
  $protectedTrustRootPaths = @('config/independent-review-keys.json', 'config/trusted-supervisor-keys.json', 'scripts/verify-independent-review.mjs', 'scripts/materialize-independent-review-receipt.ps1', 'scripts/trusted-merge-gate.ps1', 'scripts/bootstrap-trust-root.ps1', 'scripts/agent-pr-gate.ps1', 'scripts/agent-pr-gate-common.psm1', 'scripts/agent-git-common.psm1')
