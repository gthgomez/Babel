...
function Resolve-AgentWorktreeRoot {
  ...
  if (Test-Path -LiteralPath $ConfigPath -PathType Leaf) {
    ...
    # Only the shared-config layout embeds the clone directory name, so only
    # this branch requires it to be a safe single path segment. Explicit roots
    # and the clone-local fallback must keep working for clones whose directory
    # name contains spaces or other non-segment characters.
    Assert-AgentSafeSegment -Value $RepositorySlug -Label 'Repository name'
  ...
  # public fallback: <clone>/../worktrees/Babel, exactly as before this change.
  # It never discovers or depends on a private parent tree.
