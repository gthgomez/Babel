...
## 9. OUTPUT CONTRACT (ALL MODELS — MANDATORY)

These rules apply regardless of which model is executing this stage.

**JSON format — no exceptions:**
- Your entire response is a single JSON object `{...}`. No prose before or after it. No markdown fences.
- Start with `{`. End with `}`. Nothing outside those braces.

**Field types:**
- `verdict`: exactly `"PASS"` or `"REJECT"` — no other values, no casing variants.
...
1. Is my output a single `{...}` block with no surrounding text? If not, strip everything outside it.
2. Is `overall_confidence` an integer without a decimal point? If not, round down.
3. Does `failure_count` equal the number of items in `failures`? If not, correct the count.
4. Is `verdict` exactly `"PASS"` or `"REJECT"`? If not, fix casing.
